 ...
 * Project: dEdxRecon
 *
 * Brief: Implementation of the Pad class which holds waveform data and
 *        identifiers for a single detector pad. Provides accessors and
 *        small helpers used by clustering and fitting routines.
 *
 * Contents: Pad constructor/destructor and waveform/access methods.
 *
 * Notes: See Pad.h for the public API.
 ***************************************************************************/
...
Pad::Pad(Model_ReadOutGeometry *pModel_ReadOutGeometry,
  ...
  double Ypad_H = 0.;
  double Ypad_L = 0.;
  p_Model_ReadOutGeometry->GetPadEdges(m_iX, m_iY, ModuleNber, Xpad_L, Xpad_H,
                                       Ypad_L, Ypad_H);
...
Pad::~Pad() {
  int VerboseDelete = 0;
  if (VerboseDelete == 1)
    std::cout << " Pad Destructor "
              << " m_iX " << m_iX << " m_iY " << m_iY << std::endl;
  ...
  //
  v_ADC.clear();
  std::vector<short int>().swap(v_ADC);
  v_iTimeb.clear();
  std::vector<short int>().swap(v_iTimeb);
  //
}

const Model_ReadOutGeometry *Pad::Get_Model_ReadOutGeometry() const {
  return p_Model_ReadOutGeometry;
}
const Model_Electronics *Pad::Get_Model_Electronics() const {
...
void Pad::Set_AMax(const double &AMax) {
  m_AMax = AMax;
  m_AMax_FromSet = AMax;
}
void Pad::Set_TMax(const double &TMax) {
  m_TMax = TMax;
  m_TMax_FromSet = TMax;
}

void Pad::SetEdges(const double &XL, const double &XH, const double &YL,
                   const double &YH) {
  ...
  if (m_LX <= 0. || m_LY <= 0.) {
    ...
    std::cout << "	m_XPad " << std::setw(13) << std::setprecision(6)
              << m_XPad << std::endl;
    std::cout << "	m_YPad " << std::setw(13) << std::setprecision(6)
              << m_YPad << std::endl;
    std::cout << "	m_LX " << std::setw(13) << std::setprecision(6) << m_LX
              << std::endl;
    std::cout << "	m_LY " << std::setw(13) << std::setprecision(6) << m_LY
              << std::endl;
    std::cout << "	m_XL " << std::setw(13) << std::setprecision(6) << m_XL
              << std::endl;
    std::cout << "	m_XH " << std::setw(13) << std::setprecision(6) << m_XH
              << std::endl;
    std::cout << "	m_YL " << std::setw(13) << std::setprecision(6) << m_YL
              << std::endl;
    std::cout << "	m_YH " << std::setw(13) << std::setprecision(6) << m_YH
              << std::endl;
...
void Pad::WriteOut() const {
  ...
  std::cout << " m_AMax_WF; " << std::setw(16) << std::setprecision(9)
            << m_AMax_WF << std::endl;
  std::cout << " m_TMax_WF; " << std::setw(16) << std::setprecision(9)
            << m_TMax_WF << std::endl;
...
void Pad::Get_A_T_Max_WF_01(double &Amax, double &Tmax) {
  ...
  if (Tmax == 0)
    Get_A_T_Max_WF(Amax, Tmax); // Prevent absurd records
}
...
void Pad::Get_A_T_Max_FIT(double &Amax, double &Tmax) {
  ...
  TH1F *pTH1F = new TH1F("BIDONGet_A_T_Max_FIT", "BIDONGet_A_T_Max_FIT", 510,
                         -0.5, 509.5);
  ...
  for (Int_t iTimeBin = 0; iTimeBin < NADC; iTimeBin++) {
    ...
    if (ADC_value < 0)
      continue;
    int iTimeBinLoc = iTimeBin + 1; // NB: the 1st Data (iTimeBin=0) goes in the
                                    // 1st bin (iTimeBinLoc=1) [-0.5,+0.5]
  ...
  TF1 *pTF1 =
      new TF1("ParabolaFunction", aParabolaFunction, T_FitMin, T_FitMax, 3);
  ...
  int Loc_FIT_Status_A =
      pTH1F->Fit("ParabolaFunction", "Q", "", T_FitMin, T_FitMax);
  ...
  if (Loc_FIT_Status_A == 0) {
    ...
    if (Loc_FIT_Status_B == 0) {
      ...
      TF1 *pTF12 = new TF1("ParabolaFunctionNG", aParabolaFunctionNG, T_FitMin,
                           T_FitMax, 4);
...
//------------------------------Signal Simulation-----------------------//
void Pad::SetSignalModel(const double &Time0, const double &XTrue,
                         const double &YTrue) {
  m_Time0 = Time0;
  m_XTrue = XTrue;
  m_YTrue = YTrue;
...
double Pad::Get_Qpad(const double &Time) {
  return (p_Model_ChargeI->Get_Qpad(Time, m_Time0, m_XTrue, m_YTrue, m_XL, m_XH,
                                    m_YL, m_YH));
}
...
double Pad::Get_APad(const double &Time) {
  ...
  for (int iTem = 0; iTem < m_NberOfTimeConvoPoints; iTem++) {
    ...
    ToBeReturned +=
        p_Model_ChargeI->Get_Qpad(Taui, 0., m_XTrue, m_YTrue, m_XL, m_XH, m_YL,
                                  m_YH) *
