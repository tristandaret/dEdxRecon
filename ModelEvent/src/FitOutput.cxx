 ...
 * Project: dEdxRecon
 *
 * Brief: Implementation of the FitOutput container used to store results
 *        from minimisation and fitting procedures: parameter values, errors
 *        and covariance matrices.
 *
 * Contents: FitOutput methods for setting and printing fit results.
 *
 * Notes: See FitOutput.h for the interface.
 ***************************************************************************/
...
void FitOutput::Set(const int &NberOfModelParameters) {
  ...
  //			<< " m_NberOfModelParameters " <<
  //m_NberOfModelParameters << std::endl;
...
void FitOutput::PrintFitOutput() {
  ...
  std::cout << "-Number of Model Parameters " << m_NberOfModelParameters
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "-Best Chi2 " << std::setw(20) << std::setprecision(10)
            << m_MinnLL << std::endl;
  std::cout << std::endl;
  ...
  for (int ipar = 0; ipar < m_NberOfModelParameters; ipar++) {
    std::cout << " " << ipar << " " << p_parName[ipar] << " " << std::setw(20)
              << std::setprecision(10) << p_par[ipar] << " +/-" << std::setw(20)
              << std::setprecision(10)
              << p_eparparab[ipar]
              //				<< " ( "
              //				<< std::setw(20) <<
              //std::setprecision(10) << p_eparplus	[ipar]
              //				<< "; "
              //				<< std::setw(20) <<
              //std::setprecision(10) << p_eparminus [ipar]
              //				<< " ) "
              << std::endl;
  }
  std::cout << std::endl;
...
void FitOutput::PrintFitOutputInCanvas(double Xstart, double Ystart,
                                       double Step) {
  TLatex aTLatex;
  ...
  aTLatex.DrawLatex(Xstart, (Ystart - 1 * Step),
                    (Aostringstream.str()).c_str());

  Aostringstream.str("");
  Aostringstream.clear();
  Aostringstream << "-Description " << m_Description;
  aTLatex.DrawLatex(Xstart, (Ystart - 2 * Step),
                    (Aostringstream.str()).c_str());

  Aostringstream.str("");
  Aostringstream.clear();
  Aostringstream << "-Number of Model Parameters " << m_NberOfModelParameters;
  aTLatex.DrawLatex(Xstart, (Ystart - 3 * Step),
                    (Aostringstream.str()).c_str());

  Aostringstream.str("");
  Aostringstream.clear();
  Aostringstream << "-Number of Data points " << m_NberOfDataPoints;
  aTLatex.DrawLatex(Xstart, (Ystart - 4 * Step),
                    (Aostringstream.str()).c_str());

  Aostringstream.str("");
  Aostringstream.clear();
  Aostringstream << "-Best Chi2 " << std::setw(20) << std::setprecision(10)
                 << m_MinnLL;
  aTLatex.DrawLatex(Xstart, (Ystart - 5 * Step),
                    (Aostringstream.str()).c_str());

  Aostringstream.str("");
  Aostringstream.clear();
  Aostringstream << "-Parameters: name, values and errors ";
  aTLatex.DrawLatex(Xstart, (Ystart - 6 * Step),
                    (Aostringstream.str()).c_str());

  for (int ipar = 0; ipar < m_NberOfModelParameters; ipar++) {
    Aostringstream.str("");
    Aostringstream.clear();
    Aostringstream << " " << ipar << " " << p_parName[ipar] << " "
                   << std::setw(20) << std::setprecision(10) << p_par[ipar]
...
void FitOutput::SetResults(TVirtualFitter *pTVirtualFitter) {
  ...
  for (int ipar = 0; ipar < m_NberOfModelParameters; ipar++) {
    ...
    if (ier2 < 0)
      std::cout << " ier2 " << ier2 << " for pTVirtualFitter->GetErrors "
                << std::endl;
