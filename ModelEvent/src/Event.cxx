 ...
 * Project: dEdxRecon
 *
 * Brief: Implementation of the Event class which aggregates modules,
 *        clusters and tracks for a single detector event. Provides methods
 *        for validation, module management and access to underlying models
 *        (geometry, electronics, charge).
 *
 * Contents: Event constructor/destructor, validation and module handling.
 *
 * Notes: See Event.h for details of the API and ownership semantics.
 ***************************************************************************/
...
Event::~Event() {
  ...
  if (VerboseDelete == 1)
    std::cout << " Event Destructor "
              << " m_EventNber " << m_EventNber << " m_EntryNber "
              << m_EntryNber << std::endl;
  Clear_Modules();
}
...
void Event::Clear_Modules() {
  ...
  //
  V_Module.clear();
  std::vector<Module *>().swap(V_Module);
  //
}
...
Module *Event::Get_Module_InArray(const int &Index1D) {
  return V_Module[Index1D];
}

//----------------------------Clusters----------------------//
std::vector<Cluster *>
Event::GiveMe_Clusters_ForThisModule(const int &ModuleNber) {
...
double Event::GiveMe_CutSlopeXY_ForThisModule(const int &ModuleNber) {
  ...
  double SlopeXY =
      36. * (XY_Mean - X_Mean * Y_Mean) / (XX_Mean - X_Mean * X_Mean);
...
//---------------------------------------------------------------------------------
std::vector<double>
Event::GiveMe_CutSlopeXYZ_ForThisModule(const int &ModuleNber) {
  TGraph gr;
  TGraph gr1;
  ...
  if (EventNber < 1000)
    xzinfo.SaveAs(TString::Format(
        "/feynman/home/dphp/shassani/work/Analysis_3.06/"
...
Cluster *Event::Get_Cluster_Copy(Cluster *pCluster) {
  Cluster *ToBeReturned =
      new Cluster(pCluster->Get_EventNber(), pCluster->Get_EntryNber(),
...
int Event::Validity_ForThisModule(const int &ModuleNber) const {
  ...
  if (pModule) {
    ...
    // std::cout << "WARNING from Event::Validity_ForThisModule: Module #" <<
    // ModuleNber
...
void Event::Validate_ThisModule(const int &ModuleNber) {
  ...
  // else{
  //	std::cout << "WARNING from Event::Validate_ThisModule: Module #" <<
  //ModuleNber << "
  // does not exist" << std::endl;
  // }
}
void Event::Invalidate_ThisModule(const int &ModuleNber) {
  ...
  // else{
  //	std::cout << "WARNING from Event::Invalidate_ThisModule: Module #" <<
  //ModuleNber << " does not exist" <<
  // std::endl;
  // }
}
...
//----------------------------Models----------------------//
const Model_ReadOutGeometry *Event::Get_Model_ReadOutGeometry() {
  return p_Model_ReadOutGeometry;
}
const Model_Electronics *Event::Get_Model_Electronics() {
