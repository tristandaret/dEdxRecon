 ...
 * Project: dEdxRecon
 *
 * Brief: Implementation of the ROBoard abstraction which maps readout
 *        hardware channels to pad indices and groups pads into board-level
 *        structures used by uploaders and I/O modules.
 *
 * Contents: ROBoard constructor/destructor and mapping helpers.
 *
 * Notes: See ROBoard.h for the public API.
 ***************************************************************************/
...
ROBoard::~ROBoard() {
  int VerboseDelete = 0;
  if (VerboseDelete == 1)
    std::cout << " ROBoard Destructor " << std::endl;
  ...
  V_ExisFlag2D.clear();
  std::vector<int>().swap(V_ExisFlag2D);
  //
}

const Model_ReadOutGeometry *ROBoard::Get_Model_ReadOutGeometry() {
  return p_Model_ReadOutGeometry;
}
const Model_Electronics *ROBoard::Get_Model_Electronics() {
...
int ROBoard::GetLinearIndex(const int &iX, const int &iY) const {
  return (iX + iY * m_Nx);
}
...
int ROBoard::Get_IsThisPadExisting(const int &iX, const int &iY) const {
  if (iX < 0 || iX >= m_Nx || iY < 0 || iY >= m_Ny)
    return 0;
  if (V_ExisFlag2D[GetLinearIndex(iX, iY)] == 0)
    return 0;
  return 1;
}

Pad *ROBoard::Get_Pad(const int &iX, const int &iY) {
  if (Get_IsThisPadExisting(iX, iY) == 0)
    return 0;
...
void ROBoard::Ini_Models(Model_ReadOutGeometry *pModel_ReadOutGeometry,
  ...
  //
  V_ExisFlag2D.clear();
  V_ExisFlag2D.resize(m_Nx * m_Ny, 0);
}
