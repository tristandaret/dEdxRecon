#/***************************************************************************
 * File: SetOfTracks.cxx
 * Project: dEdxRecon
 *
 * Brief: Implementation of the SetOfTracks container used to store and
 *        operate on collections of Track objects used by reconstruction
 *        and analysis routines.
 *
 * Contents: SetOfTracks methods for adding, removing and querying tracks.
 *
 * Notes: Interface declared in SetOfTracks.h.
 ***************************************************************************/
...
SetOfTracks::SetOfTracks() { m_MinimalNberOfEntries = 100; }
...
void SetOfTracks::Add_Track(const Track *pTrack) { V_Track.push_back(pTrack); }

int SetOfTracks::Get_NberOfTrack() const { return V_Track.size(); }
...
int SetOfTracks::Get_MinimalNberOfEntries() { return m_MinimalNberOfEntries; }
void SetOfTracks::Set_MinimalNberOfEntries(const int &MinimalNberOfEntries) {
  m_MinimalNberOfEntries = MinimalNberOfEntries;
}

TH1F *SetOfTracks::GiveMe_pTH1F_Ch2Min(const std::string &TAGFinal,
                                       const double &Xmax) {
...
TH1F *SetOfTracks::GiveMe_pTH1F_Ch2MinPerNODF(const std::string &TAGFinal,
                                              const double &Xmax) {
  ...
  aostringstream << Xmax;
  std::string Name =
      "GiveMe_pTH1F_Ch2MinPerNODF" + TAGFinal + aostringstream.str();
  ...
  for (int iT = 0; iT < NberOfTracks; iT++) {
    ...
    double Ch2MinPerDOF =
        pTrack->Get_Chi2Min() /
        double(pTrack->Get_NberOfCluster() - pTrack->GetNberOfParameters());
    ToBeReturned->Fill(Ch2MinPerDOF);
  }
  return ToBeReturned;
}

TH1F *SetOfTracks::GiveMe_pTH1F_Residual(const std::string &TAGFinal,
  ...
  for (int iTurn = 0; iTurn < 3; iTurn++) {
    ...
    if (fitStatus != 0) {
      std::cout << " fitStatus = " << fitStatus << " Name = " << Name
                ...
                << " L_Norm = " << L_Norm
                << " L_Norm Scaled = " << L_Norm * (XmaxH - XminH) / 100.
                << std::endl;
    }
...
TH1F *SetOfTracks::GiveMe_pTH1F_Residual(const std::string &TAGFinal,
                                         const int &iX, double &Mean,
                                         double &eMean, double &Sigma,
                                         double &eSigma) {
  ...
  aostringstream2 << iX;
  std::string Name =
      "GiveMe_pTH1F_ResidualV1" + TAGFinal + aostringstream2.str();
...
TH1F *SetOfTracks::GiveMe_pTH1F_Residual(const std::string &TAGFinal,
                                         const int &iX, double YL, double YH,
                                         double YL_Rescaled, double YH_Rescaled,
                                         double &Mean, double &eMean,
                                         double &Sigma, double &eSigma) {
  ...
  aostringstream2 << YH;
  std::string Name =
      "GiveMe_pTH1F_ResidualV2" + TAGFinal + aostringstream2.str();

  std::ostringstream aostringstream;
  aostringstream << std::setiosflags(std::ios::fixed);
  aostringstream << "Track fit : Residual (mm) : iX " << iX;
  aostringstream << "- Range " << std::setw(8) << std::setprecision(6)
                 << YL_Rescaled;
  aostringstream << "-> " << std::setw(8) << std::setprecision(6)
                 << YH_Rescaled;
  std::string Title = aostringstream.str();
...
//-----------------------------------------------------------------//
TH1F *SetOfTracks::GiveMe_pTH1F_TrackDeltaT(const std::string &TAG,
                                            const int &iCol) {
  std::ostringstream aostringstream;
  aostringstream << std::setiosflags(std::ios::fixed);
  ...
  for (int iT = 0; iT < NberOfTracks; iT++) {
    ...
    for (int iX = 0; iX < NX; iX++) {
      if (iX != iCol && iCol != -1)
        continue;
      double X0 =
          pModel_ReadOutGeometry->Get_XcPad(iX, 0, pTrack->Get_ModuleNber());
      double YTtrack = pTrack->Y_Position(X0);

      int iY0 = std::floor((YTtrack - Ypad_min) / LY);
      double Y0 =
          pModel_ReadOutGeometry->Get_YcPad(iX, iY0, pTrack->Get_ModuleNber());
...
//-----------------------------------------------------------------//
TH1F *
SetOfTracks::GiveMe_pTH1F_YTrackYPadLeadingInTracks(const std::string &TAG,
                                                    const int &iCol) {
  std::string SubTAG = TAG + "Indirect";
  return GiveMe_pTH1F_YTrackYPadLeadingInTracks(SubTAG, iCol, 100, -3., 3.);
}
TH1F *SetOfTracks::GiveMe_pTH1F_YTrackYPadLeadingInTracks(
    const std::string &TAG, const int &iCol, const int &Nbins,
    const double &Xmin, const double &Xmax) {
  std::ostringstream aostringstream;
  aostringstream << std::setiosflags(std::ios::fixed);
  aostringstream << "GiveMe_pTH1F_YTrackYPadLeadingInTracks" << TAG << "COL"
                 << iCol;
  ...
  std::ostringstream aostringstream2;
  aostringstream2 << std::setiosflags(std::ios::fixed);
  aostringstream2 << " Y^{Cluster}_{Track}-Y_{Pad}^{Leading} (cm) at col. "
                  << iCol;
...
TH1F *SetOfTracks::GiveMe_pTH1F_YFitCol(const std::string &TAG,
  ...
  double Ycen = 1.E2 * pModel_ReadOutGeometry->Get_YcPad(
                           0, iYBeam, pTrack->Get_ModuleNber());
  double Ymin = Ycen - 5.;
  double Ymax = Ycen + 5.;
...
//-----------------------------------------------------------------//
TH1F *SetOfTracks::GiveMe_pTH1F_YTrackInTracks(const std::string &TAG,
                                               const int &iYBeam) {
  std::string SubTAG = TAG + "Indirect1";
