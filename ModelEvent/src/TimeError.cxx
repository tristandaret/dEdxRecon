 ...
 * Project: dEdxRecon
 *
 * Brief: Implementation of time-error helper functions that estimate
 *        uncertainties on timing measurements for pads and pulses. These
 *        are used when weighting fits or propagating timing uncertainties.
 *
 * Contents: TimeError function implementations.
 *
 * Notes: See TimeError.h for function signatures.
 ***************************************************************************/
...
double TimeError_BASE(const double &DeltaT_in) {
  ...
  double Y2 = 10.;
  if (DeltaT_in <= X2)
    return ((DeltaT_in - X1) * (Y2 - Y1) / (X2 - X1) + Y1);
