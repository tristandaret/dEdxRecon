...
public:
  ...
  //------------------------------Track Access-----------------------//
  SetOfTracks *Get_SetOfTracks_ForThisModule(
      const int &ModuleNber); // Ownership is passed to user

  //------------------------------PRF from input file-----------------------//
  void SetFilePRF(const double &Norm, const double &a2, const double &a4,
                  const double &b2, const double &b4);

  void GetFilePRF(int &FilePRF_Exist, double &Norm, double &a2, double &a4,
                  double &b2, double &b4);
