...
FitOutput::FitOutput(const FitOutput& ToBeCopied)
{
  ...
  if (ToBeCopied.p_CovMatrix){
    for ( int Item = 0 ; Item<NdimSquared ; Item++) p_CovMatrix [Item] = ToBeCopied.p_CovMatrix [Item];
  }else{
    for ( int Item = 0 ; Item<NdimSquared ; Item++) p_CovMatrix [Item] = 0. ;
    std::cout << "===> FitOutput::FitOutput:  "
              << "No covarance matrix available "
              << std::endl ;
  }  
...
void FitOutput::PrintFitOutput()
{
  ...
  for (int ipar = 0 ; ipar<m_NberOfModelParameters ; ipar++) {
    std::cout << " "    << ipar
              ...
              << " +/-" << std::setw(20) << std::setprecision(10) << p_eparparab[ipar]
//               << " ( "  
...
void FitOutput::PrintFitOutputInCanvas(double Xstart, double Ystart,double Step)
{
   ...
   Aostringstream.str("");Aostringstream.clear();
...
//             << " m_NberOfModelParameters " << m_NberOfModelParameters << std::endl;
  ...
  for (int ipar = 0 ; ipar<m_NberOfModelParameters ; ipar++) {
    ...
    if (ier2 < 0 ) std::cout << " ier2 " << ier2 << " for pTVirtualFitter->GetParameter " << std::endl;
    
    double eplus,eminus,eparab,globcc  ;
    ier2 = pTVirtualFitter->GetErrors(ipar,eplus,eminus,eparab,globcc);
    if (ier2 < 0 ) std::cout << " ier2 " << ier2 << " for pTVirtualFitter->GetErrors " << std::endl;
...
void FitOutput::SetResults   (
  ...
  for (int ipar = 0 ; ipar<m_NberOfModelParameters ; ipar++) {
    for (int jpar = 0 ; jpar<m_NberOfModelParameters ; jpar++) {
      if (ipar!=jpar) p_CovMatrix[ipar + jpar*m_NberOfModelParameters] = 0. ;
      if (ipar==jpar) p_CovMatrix[ipar + jpar*m_NberOfModelParameters] = 1./( p_eparplus[ipar]*p_eparplus[ipar]) ;
    }
  }
