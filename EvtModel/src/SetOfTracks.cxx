...
TH1F* SetOfTracks::GiveMe_pTH1F_Ch2Min  (   
...
TH1F* SetOfTracks::GiveMe_pTH1F_Ch2MinPerNODF  (   
                                   const std::string& TAGFinal ,
                                   const double& Xmax
){
  ...
  for (int iT = 0 ; iT< NberOfTracks; iT++){
    const Track* pTrack = Get_Track(iT) ;
    double Ch2MinPerDOF = 
    pTrack->Get_Chi2Min ()/double( pTrack->Get_NberOfCluster() -  pTrack->GetNberOfParameters() )  ;
    ToBeReturned->Fill( Ch2MinPerDOF ) ;
  }
  return ToBeReturned ;
}

TH1F* SetOfTracks::GiveMe_pTH1F_Residual (
                                    const std::string& TAGFinal ,
                                    double& Mean   , 
                                    double& eMean  , 
                                    double& Sigma  , 
...
TH1F* SetOfTracks::GiveMe_pTH1F_Pull (
                                    const std::string& TAGFinal ,
                                    double& Mean   , 
                                    double& eMean  , 
                                    double& Sigma  , 
    ...
    for (int iC = 0 ; iC< NberOfClusters; iC++){
      ...
      if (iXCluster==iX){
        ...
        if ( YL < Position && Position <= YH ){
          double Residual = pTrack->Get_Residual(iC) *1.E3;
          if ( std::fabs(Residual) < 1.5 )V_Data.push_back( Residual );
        }
      }
    }
  }
...
//-----------------------------------------------------------------//
TH1F* SetOfTracks::GiveMe_pTH1F_TrackDeltaT(   
  ...
  if (iCol==-1) Title = " #Delta_{T} (cm) all col. "  ; 
...
//-----------------------------------------------------------------//
TH1F* SetOfTracks::GiveMe_pTH1F_YTrackYPadInTracks ( 
                                             const std::string& TAG,
                                             const int& iCol
){
...
TH1F* SetOfTracks::GiveMe_pTH1F_YTrackYPadInTracks ( 
  ...
  std::string Title = aostringstream2.str()  ; 
  if (iCol==-1) Title = " Y^{Cluster}_{Track}-Y_{Pad} (cm) all col. "  ; 
...
TH1F* SetOfTracks::GiveMe_pTH1F_YTrackYPadLeadingInTracks ( 
  ...
  std::string Title = aostringstream2.str()  ; 
  if (iCol==-1) Title = " Y^{Cluster}_{Track}-Y_{Pad}^{Leading} (cm) all col. "  ; 
...
//-----------------------------------------------------------------//
TH1F* SetOfTracks::GiveMe_pTH1F_YFitCol (   
                                        const std::string& TAG,
                                        const int& iYBeam
){
...
TH1F* SetOfTracks::GiveMe_pTH1F_YFitCol (   
  ...
  std::string Title = aostringstream2.str()  ; 
  if (iCol==-1) Title = " Track Y_{T} (cm) all col. "  ; 
...
//-----------------------------------------------------------------//
TH1F* SetOfTracks::GiveMe_pTH1F_YTrackInTracks (   
                                  const std::string& TAG,
                                  const int& iYBeam
){
  std::string SubTAG = TAG +"Indirect1" ;
...
TH1F* SetOfTracks::GiveMe_pTH1F_YTrackInTracks (   
  ...
  std::string Title = aostringstream2.str()  ; 
  if (iCol==-1) Title = " Cluster Y_{T} (cm) all col. "  ; 
