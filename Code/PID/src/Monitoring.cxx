...
void PID::Monitoring()
{
  ...
  if(DESY_phi){
    ...
    for (int iz = 0; iz < (int)std::size(z_arr); iz++){
      ...
      if(DO_Resolution)   DrawOut_Phiscan (Form("OUT_PID/DESY21_phi/DESY21_phi_z%s", z), comment, "_WF" + WFversion, z);
      if(DO_Systematics)  DrawOut_Systematics(outDir, comment, "phi");
    }
    // DrawOut_TGE_WFsum_L("OUT_PID/DESY21_phi/", comment);
    if(DO_Global) DrawOut_Phiscan_Z("OUT_PID/DESY21_phi", comment, "_WF" + WFversion);
  }
  ...
  if(DO_Scans) DrawOut_Scans("OUT_PID", comment, "_WF" + WFversion);
