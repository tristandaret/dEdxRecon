...
//
  ...
  int KountEvent_NotEmpty_Max = 10000 ;

  std::string EventFile = "BIDON" ;
  int iFile_Min = 0 ;
  int iFile_Max = 1 ;
  for (int iFile = iFile_Min ; iFile< iFile_Max; iFile++){
//  if (iFile==0)  EventFile = "../Real_Data/DAVID/All_ERAMS_350V_412ns_e+_0p5GeV_25V_z415_y2pad_iter0.root" ;
//  if (iFile==0)  EventFile = "../Real_Data/COSMICS23/TEST/R2022_10_29-15_59_15-000_iter0.root" ;
    if (iFile==0)  EventFile = "../Real_Data/COSMICS23/TEST/run00002_00_iter0.root" ;
...
//
    std::cout << std::endl ;
    std::cout << " DIA_DAVID_01 " << std::endl ;
...
//  T Leading Histos
    ...
    for (int iMod = 0; iMod < 8; iMod++) {
      ...
      std::ostringstream aostringstreamTITLE_1 ;
      aostringstreamTITLE_1 << std::setiosflags(std::ios::fixed) ;
      aostringstreamTITLE_1 << "TLeading (zoom): Mod "  << iMod;
      std::string  Title_1 =  aostringstreamTITLE_1.str() ;

      std::ostringstream aostringstreamTITLE_2 ;
      aostringstreamTITLE_2 << std::setiosflags(std::ios::fixed) ;
      aostringstreamTITLE_2 << "TLeading : Mod "  << iMod;
      std::string  Title_2 =  aostringstreamTITLE_2.str() ;

      TH1F* pTH1_1 = new TH1F(NAME_A.c_str(),Title_1.c_str(),11,  -0.5 , 10.5 );
      TH1F* pTH1_2 = new TH1F(NAME_B.c_str(),Title_2.c_str(),511,  -0.5 , 510.5 );
...
//
    ...
    std::cout << "      NberOfEvent =  " << NberOfEvent << std::endl ;
...
//  Loop On Events
    int KountEvent          = 0 ;
    int KountEvent_NotEmpty = 0 ;
    
    for (int iEvent = 0; iEvent < NberOfEvent; iEvent++) {
      std::cout << " iEvent " << iEvent << std::endl ; 
      if (KountEvent_NotEmpty>= KountEvent_NotEmpty_Max ) break ;
      KountEvent += 1 ;
...
//    Stat of Modules in events    
      ...
      for (int iMod = 0; iMod < 8; iMod++) {
        ...
        if (v_IOK_Module[iMod] == 1 ) {
          ...
          pTH1_Module->Fill( double(iMod) , 1. ) ;
        }
      ...
      pTH1_Logic0123->Fill( double(Logic0123) , 1. ) ;
      
      if (Logic0123>=1) pTH1_Logic0123_Inc->Fill( 1. , 1. ) ;
      if (Logic0123>=2) pTH1_Logic0123_Inc->Fill( 2. , 1. ) ;
      if (Logic0123>=3) pTH1_Logic0123_Inc->Fill( 3. , 1. ) ;
      if (Logic0123>=4) pTH1_Logic0123_Inc->Fill( 4. , 1. ) ;

      double Logic4567 = v_IOK_Module[4] + v_IOK_Module[5] + v_IOK_Module[6] + v_IOK_Module[7] ;
      pTH1_Logic4567->Fill( double(Logic4567) , 1. ) ;

      if (Logic4567>=1) pTH1_Logic4567_Inc->Fill( 1. , 1. ) ;
      if (Logic4567>=2) pTH1_Logic4567_Inc->Fill( 2. , 1. ) ;
      if (Logic4567>=3) pTH1_Logic4567_Inc->Fill( 3. , 1. ) ;
      if (Logic4567>=4) pTH1_Logic4567_Inc->Fill( 4. , 1. ) ;
...
//
    ...
    std::cout << std::endl ;
    std::cout << " Nber Of Entries in total :" << NberOfEvent << std::endl ;
    std::cout << " Nber Of Events examined  :" << KountEvent << std::endl ;
    std::cout << " Nber Of Not Empty Events :" << KountEvent_NotEmpty << std::endl ;
    std::cout << std::endl ;
...
//  Page 2
    pTCanvas = new TCanvas("Bla", "Bla", 180, 10, 1800, 1200) ;
      ...
      pTH1_Module->Scale(100./double(KountEvent_NotEmpty));
...
//  Page 3
    pTCanvas = new TCanvas("Bla", "Bla", 180, 10, 1800, 1200) ;
      ...
      pTCanvas->cd(1);
        ...
        pTH1_Logic0123->Scale(100./double(KountEvent_NotEmpty));
      ...
      pTCanvas->cd(2);
        ...
        pTH1_Logic4567->Scale(100./double(KountEvent_NotEmpty));
      ...
      pTCanvas->cd(3);
        ...
        pTH1_Logic0123_Inc->Scale(100./double(KountEvent_NotEmpty));
      ...
      pTCanvas->cd(4);
        ...
        pTH1_Logic4567_Inc->Scale(100./double(KountEvent_NotEmpty));
