...
{ 
  ...
  TH1F* h1f_WFtrunc         = new TH1F("h1f_WFtrunc",         "<dE/dx> with WF_{cluster} (module %i);<dE/dx> (ADC count);Number of events", 90, 0, 1800) ;
  ...
  for (int iEvent = 0 ; iEvent < NEvent ; iEvent++){
    ...
    // WFtrunc                             = accumulate(v_WF.begin(), v_WF.end(), 0.)/(len_track*(N_clus_trc/N_clus)) ;
    ...
    std::sort(v_WF.begin(), v_WF.end()) ;
    std::sort(v_rank_WF.begin(), v_rank_WF.end()) ;
    WFtrunc                             = std::accumulate(v_WF.begin(), v_WF.begin() + N_cross_clust_trc_Evt, 0.) / N_cross_clust_trc_Evt ;
    h1f_WFtrunc->                         Fill(WFtrunc) ;
    for(int i=0; i<N_cross_clust_trc_Evt; i++)  h2f_WFtruncvsLength->Fill(v_len_clust_Evt[v_rank_WF[i].Rank]*1000, v_WFmax[v_rank_WF[i].Rank]) ;
  ...
  TF1* tf1_WFtrunc                    = Fit1Gauss(h1f_WFtrunc, 2) ;
  tf1_WFtrunc->                         SetNameTitle("tf1_WFtrunc", "tf1_WFtrunc") ;
