...
void combinedFit(std::vector<TGraphErrors*>& v_tge, std::vector<TF1*>& v_tf1)
{
  ...
  //  const char* formula    = "[1]/pow(x/sqrt(x*x+[0]*[0]),[4]) * ( [2] - pow(x/sqrt(x*x+[0]*[0]),[4]) - log([3]+ pow(x*x/([0]*[0]), [5])) )" ;
   ...
   if(nparticles == 4){
    ...
    v_tf1[0]->              FixParameter(0, 511e-3);
   }else{
    v_tf1[0]              = BetheBlochExp(0.45, 1.55, 511e-6, "positrons");
    v_tge[0]->              Fit(v_tf1[0], "R", "", 0.45, 1.55);
   }
  ...
  //  GlobalChi2_4 globalChi2_4(chi2_muon, chi2_pion, chi2_proton, chi2_positron);
   ...
   if(nparticles < 4) fitter.Config().SetParamsSettings(8, par0);
   // fix parameters
   fitter.Config().ParSettings(5).Fix();
   fitter.Config().ParSettings(6).Fix();
   fitter.Config().ParSettings(7).Fix();
   if(nparticles == 4) fitter.Config().ParSettings(8).Fix();
  //  fitter.Config().ParSettings(3).SetLimits(1, 3);
   ...
   if(nparticles < 4)   fitter.FitFCN(8, globalChi2_3, 0, datamuon.Size() + datapion.Size() + dataproton.Size(), true);
  //  else                 fitter.FitFCN(9, globalChi2_4, 0, datapositron.Size() + datamuon.Size() + datapion.Size() + dataproton.Size(), true);
   ...
   v_tge[1]->GetXaxis()->SetLimits(0.45, 1.55) ;
   ...
   c1->SaveAs(Form("Test_Simultaneous_fit_%i.pdf", nparticles));
}