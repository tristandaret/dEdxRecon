...
void DrawOut_EventDisplay(Module* pModule, const std::string& OUTDIR,const std::string& TAG, const std::string& type, const double& parabola, const double& slope, const double& intercept)
{
  ...
  if(type == "time"){
    ...
    OutputFile  = OUTDIR + "Time_Display_Entry_" + std::to_string(pModule->Get_EntryNber()) + "_Evt_" + std::to_string(pModule->Get_EventNber()) + "_Mod_" + std::to_string(pModule->Get_ModuleNber()) + ".png" ;
  ...
  if(slope != 999 && intercept != 999){
    pTF1_Track->            SetParameters((intercept*100-0.5)/1.019, slope*1.128/1.019, (parabola/100)*1.128/1.019) ;
  ...
  delete pTF1_Track ;
  delete pTCanvas_Evt ;
  if(type == "time") TColor::InvertPalette() ; // reset invertion
...
void DrawOut_ClusterWFDisplay(Cluster* pCluster,const std::string& OUTDIR,const std::string& TAG, const int& Option, const int& PT, const int& TB)
{
  ...
  aostringstream << ".png" ;
  std::string OutputFile =  aostringstream.str() ;
  std::string OutCWF = OUTDIR + TAG + "_CWF_Event_" + std::to_string(pCluster->Get_EventNber()) + "_Mod_" + std::to_string(pCluster->Get_ModuleNber()) + "_Cluster_" + std::to_string(pCluster->Get_LeadingPad()->Get_iX()) + ".png" ;
  gStyle->                          SetStatX(0.9) ;
  gStyle->                          SetStatY(0.9) ;
...
//Sum the pads WF
  ...
  for (int iP = 0 ; iP < NPads ; iP++){
    ...
    if(iP == 0){ADCminWF = ADCminPad ; ADCmaxWF = ADCmaxPad ;}
    if(ADCminPad < ADCminWF) ADCminWF = ADCminPad ;
    if(ADCmaxPad > ADCmaxWF) ADCmaxWF = ADCmaxPad ;
  }
  ...
  for (int iP = 0 ; iP < NPads ; iP++){
    ...
    if ( Option >= 2 ){
      ...
      TLegendEntry* pTLegendEntry_A = pTLegend_A->AddEntry((TObject*)0, (aostringstream_pTLegEntr_A.str()).c_str(), "");
  ...
  if ( Option >= 1 ){
    ...
    TH1F* pTH1F_DPR               = DPR("pTH1F_DPR_" + pCluster->Get_EntryNber() + pCluster->Get_LeadingPad()->Get_iX(), -0.5, 509.5, Tsum-PT/TB, 510, 999, PT, TB) ;
...
{
  std::string OutputFile          = OUTDIR + TAG + "_GigaWF_Entry_" + std::to_string(pEvent->Get_EntryNber()) + "_Event_" + std::to_string(pEvent->Get_EventNber()) + "_Mod_" + std::to_string(ModuleNber) + ".png" ;
  TH1F* pTH1F_GWF                 = new TH1F ("pTH1F_GWF", "pTH1F_GWF", 510, -0.5, 509.5) ;
  pTH1F_GWF->                       SetTitle(std::string(TAG + " Entry " + std::to_string(pEvent->Get_EntryNber()) + " Event " + std::to_string(pEvent->Get_EventNber()) + " Module " + std::to_string(pEvent->Get_Module_InArray(ModuleNber)->Get_ModuleNber()) +" Giga WaveForm (GWF);Time (/40 ns);ADC count").c_str()) ;
  ...
  for (int iC = 0 ; iC < NClusters ; iC++){
    ...
    TH1F* pTH1F_WF_DPRcluster     = DPR("pTH1F_WF_DPRcluster", -0.5, 509.5, pTH1F_cluster->GetMaximumBin()-PT/TB, 510, iC, PT, TB) ;
