#include "GenerateShape/Shape_Check_Alpha.h"
#include "Misc/Util.h"
...
void Shape_Check_Alpha()
{
  SC_VS_x_rc_dd aSC_VS_x_rc_dd("Data_SignalTime/Load_400ns_VS_x_rc_dd.csv") ;

  Shape_Check_Alpha_Fixed_DD_01(&aSC_VS_x_rc_dd) ; 
  Shape_Check_Alpha_Fixed_RC_01(&aSC_VS_x_rc_dd) ;
  Shape_Check_Alpha_Fixed_RC_02(&aSC_VS_x_rc_dd) ;

}

void Shape_Check_Alpha_Fixed_DD_01(SC_1D_VS_x_rc_dd_I* pSC_1D_VS_x_rc_dd_I)
{
...
//
  Str_Out_PDF =  "OUT_SignalShape/Shape_Check_Alpha_Fixed_DD_01.pdf" ; 
  ...
  for (int iDD = 0 ; iDD< 11 ; iDD++){
    ...
    TLegend* pTLegend = new TLegend(0.40,0.50 ,0.60,0.78  ); 
    ...
    for (int iRC = 0 ; iRC< 6 ; iRC++){
      ...
      for (int iPoint = 0 ; iPoint< Npoints ; iPoint++){
        ...
        double vCur  = pSC_1D_VS_x_rc_dd_I->Eval_AlphaPRF(vY_cur, RCcur,DDcur) ; 
...
void Shape_Check_Alpha_Fixed_RC_01(SC_1D_VS_x_rc_dd_I* pSC_1D_VS_x_rc_dd_I)
{
  Model_ReadOutGeometry       aModel_ReadOutGeometry         ;
  double LY= aModel_ReadOutGeometry.Get_LY() ;
...
//
  Str_Out_PDF =  "OUT_SignalShape/Shape_Check_Alpha_Fixed_RC_01.pdf" ; 
  Str_Out_PDF_Beg = Str_Out_PDF + "(" ;
  Str_Out_PDF_End = Str_Out_PDF + ")" ;
...
void Shape_Check_Alpha_Fixed_RC_02(SC_1D_VS_x_rc_dd_I* pSC_1D_VS_x_rc_dd_I)
{
  Model_ReadOutGeometry       aModel_ReadOutGeometry         ;
  double LY= aModel_ReadOutGeometry.Get_LY() ;
...
//
  Str_Out_PDF =  "OUT_SignalShape/Shape_Check_Alpha_Fixed_RC_02.pdf" ; 
  Str_Out_PDF_Beg = Str_Out_PDF + "(" ;
  Str_Out_PDF_End = Str_Out_PDF + ")" ;
  ...
  for (int iRC = 0 ; iRC< 6 ; iRC++){
    ...
    for (int iDD = 0 ; iDD< 11 ; iDD++){
      ...
      for (int iPoint = 0 ; iPoint< Npoints ; iPoint++){
        double vY_cur = vY_min + double (iPoint)*(vY_max-vY_min)/double(Npoints-1) ;
        double vCur  = pSC_1D_VS_x_rc_dd_I->Eval_AlphaPRF(vY_cur, RCcur,DDcur)
                     / pSC_1D_VS_x_rc_dd_I->Eval_AlphaPRF(vY_cur, RCcur,DDref); 
        vCur = (vCur-1)*100. ;            
        pTGraph ->SetPoint( pTGraph ->GetN(), vY_cur , vCur  ) ;
      }
    ...
    pTMultiGraph->GetYaxis()->SetTitle(" (#alpha -#alpha_{Ref})/ #alpha_{Ref} in %");  
    pTMultiGraph->GetYaxis()->SetTitleOffset(1.5);
    
    pTMultiGraph->SetMinimum(-1.);
    pTMultiGraph->SetMaximum( 35.);
