#include "GenerateShape/Shape_Check_TimeAbs.h"
#include "Misc/Util.h"
...
void Shape_Check_TimeAbs()
{
  ...
  Shape_Check_TimeAbs_Fixed_DD_01(&aSC_VS_x_rc_dd) ; 
  Shape_Check_TimeAbs_Fixed_RC_01(&aSC_VS_x_rc_dd) ;
...
//
  Str_Out_PDF =  "OUT_SignalShape/Shape_Check_TimeAbs_Fixed_DD_01.pdf" ; 
  ...
  for (int iDD = 0 ; iDD< 11 ; iDD++){
    ...
    for (int iRC = 0 ; iRC< 6 ; iRC++){
      ...
      for (int iPoint = 0 ; iPoint< Npoints ; iPoint++){
        ...
        double vCur  = pSC_1D_VS_x_rc_dd_I->Eval_TimeAbsPRF(vY_cur, RCcur,DDcur) ; 
    ...
    pTMultiGraph->GetYaxis()->SetTitle("T_{Pad}-T_{0} (#mu s)");  
    pTMultiGraph->GetYaxis()->SetTitleOffset(1.5);

    pTMultiGraph->SetMinimum(0.3);
    pTMultiGraph->SetMaximum(2.5);
...
//
  Str_Out_PDF =  "OUT_SignalShape/Shape_Check_TimeAbs_Fixed_RC_01.pdf" ; 
  Str_Out_PDF_Beg = Str_Out_PDF + "(" ;
  Str_Out_PDF_End = Str_Out_PDF + ")" ;
