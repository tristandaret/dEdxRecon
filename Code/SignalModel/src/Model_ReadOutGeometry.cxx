...
Model_ReadOutGeometry::~Model_ReadOutGeometry()
{
 ...
 if (VerboseDelete==1)
 std::cout 
       << " Model_ReadOutGeometry Destructor "
       << std::endl ;
}
...
void Model_ReadOutGeometry::Set_Xpad_min(const double& Xpad_min ) { m_Xpad_min = Xpad_min ; }
void Model_ReadOutGeometry::Set_Ypad_min(const double& Ypad_min ) { m_Ypad_min = Ypad_min ; }
...
double Model_ReadOutGeometry::Get_XcPad(const int& iX,const int& iY, const int& ModuleNber) const { return ( m_Xpad_min + 0.5 * m_LX + double (iX) * m_LX ) ; }
double Model_ReadOutGeometry::Get_YcPad(const int& iX,const int& iY, const int& ModuleNber) const { return ( m_Ypad_min + 0.5 * m_LY + double (iY) * m_LY ) ; }
   
double Model_ReadOutGeometry::Get_XLPad(const int& iX,const int& iY, const int& ModuleNber) const { double Xpad_C = Get_XcPad(iX,iY,ModuleNber) ; return ( Xpad_C - 0.5 * m_LX ) ; }
double Model_ReadOutGeometry::Get_YLPad(const int& iX,const int& iY, const int& ModuleNber) const { double Ypad_C = Get_YcPad(iX,iY,ModuleNber) ; return ( Ypad_C - 0.5 * m_LY ) ; }  

double Model_ReadOutGeometry::Get_XHPad(const int& iX,const int& iY, const int& ModuleNber) const { double Xpad_C = Get_XcPad(iX,iY,ModuleNber) ; return ( Xpad_C + 0.5 * m_LX ) ; } 
double Model_ReadOutGeometry::Get_YHPad(const int& iX,const int& iY, const int& ModuleNber) const { double Ypad_C = Get_YcPad(iX,iY,ModuleNber) ; return ( Ypad_C + 0.5 * m_LY ) ; } 

void Model_ReadOutGeometry::GetPadEdges(
     const int& iX , 
     const int& iY ,
