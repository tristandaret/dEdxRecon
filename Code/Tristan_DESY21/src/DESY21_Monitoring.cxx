#include "Tristan_DESY21/DESY21_Monitoring.h"
#include "Tristan_DESY21/DESY21_DrawOut_Status.h"
#include "Tristan_DESY21/DESY21_DrawOut_PRF.h"
#include "Tristan_DESY21/DESY21_DrawOut_Residuals.h"
  ...
  for(int iPV = 0  ; iPV < nPV ; iPV+= 2){
    for(int iw = 0 ; iw  < nw ; iw+= 2){
      ...
      for(int DD = 0 ; DD <= 8 ; DD++){
        ...
        dirstring << "OUT_pt" << pt << "/z" << zdrift << "/" << Stage << "/" ;
        ...
        DESY21_DrawOut_Status        (aSample, ModuleNber ,OUTDIR, std::string(TAG + "_0Raw"), Stage, iPV, 0, 0) ;
        Selection_TD_TrackAlignement  (aSample, ModuleNber , TAG, 0.5, zdrift) ;
        DESY21_DrawOut_Status        (aSample, ModuleNber ,OUTDIR, std::string(TAG + "_5Align"), Stage, iPV, 0, 0) ;
        ...
        for(int iIter = 0 ; iIter <= NIter ; iIter++){
          ...
          if(iIter == 0 or iIter == NIter) DESY21_DrawOut_Status(aSample, ModuleNber ,OUTDIR, TAG, Stage, iPV, iIter, ptf1_PRF_fit) ;  // Control plots (iter0 and iter8)
          DESY21_DrawOut_PRF(aSample, ModuleNber , OUTDIR, TAG, Stage, iPV, iIter, NIter, ptf1_PRF_fit) ;                // PRF plots (each iteration)

          // Get residuals, bias and resolution
          DoTracksReconstruction(aSample, ModuleNber ) ;
          DESY21_DrawOut_Residuals(aSample, ModuleNber , OUTDIR, TAG, ptge_std_mean, ptge_bias_mean, DD, iIter) ;
        }

        // Draw Event Displays and Waveforms if DeltaT < -100 bins
        DESY21_EventsWF(aSample, ModuleNber , TAG, Stage, pt, zdrift) ;
      }
      DrawOut_DD_functions(Stage, ptge_std_mean, ptge_bias_mean, ptge_RC, pt) ;
    }
  }
}
