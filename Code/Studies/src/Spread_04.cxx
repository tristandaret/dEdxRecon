...
//  
  TCanvas* pTCanvas =  0 ;
  std::string Str_Out_PDF     = "OUT_Studies/Spread_04.pdf" ; 
...
//
      int isOK_PR = 0 ; if (A_PR >= LowestA ) isOK_PR = 1 ;
      int isOK_PL = 0 ; if (A_PL >= LowestA ) isOK_PL = 1 ;

//
      double EC_pTGraph_Alpha_PR = std::pow(1./E_pTGraph_Alpha_PR,2) ; if (isOK_PR==0) EC_pTGraph_Alpha_PR = 0. ;
      double EC_pTGraph_Alpha_PL = std::pow(1./E_pTGraph_Alpha_PL,2) ; if (isOK_PL==0) EC_pTGraph_Alpha_PL = 0. ;
      double EC_pTGraph_Alpha = 
      std::sqrt( 
         EC_pTGraph_Alpha_PR 
       + EC_pTGraph_Alpha_PL 
      )  ;
      if ( isOK_PR==1 ||  isOK_PL==1 ){
        EC_pTGraph_Alpha = 1./EC_pTGraph_Alpha ;
      }else{
        EC_pTGraph_Alpha = Ly* 1.E6/std::sqrt(12.) ;
      }
...
//
      double EC_pTGraph_TauBin_PR = std::pow(1./E_pTGraph_TauBin_PR,2) ; if (isOK_PR==0) EC_pTGraph_TauBin_PR = 0. ;
      double EC_pTGraph_TauBin_PL = std::pow(1./E_pTGraph_TauBin_PL,2) ; if (isOK_PL==0) EC_pTGraph_TauBin_PL = 0. ;
      double EC_pTGraph_TauBin = 
      std::sqrt( 
         EC_pTGraph_TauBin_PR 
       + EC_pTGraph_TauBin_PL 
      )  ;
      if ( isOK_PR==1 ||  isOK_PL==1 ){
        EC_pTGraph_TauBin = 1./EC_pTGraph_TauBin ;
      }else{
        EC_pTGraph_TauBin = Ly* 1.E6/std::sqrt(12.) ;
      }
...
//
      ...
      if ( isOK_PR==1 ||  isOK_PL==1 ){
        ...
        EC_pTGraph_AlphaTauBin = Ly* 1.E6/std::sqrt(12.) ;
      }
...
//
  ...
  for (int iRC = 0 ; iRC< 31 ; iRC++){
    double RCcur = 0. ;
    RCcur = 50. + double(iRC)*5. ;
//     if (iRC== 0) { RCcur =  50. ; }
//     if (iRC== 1) { RCcur =  60. ; }
//     if (iRC== 2) { RCcur =  70. ; }
//     if (iRC== 3) { RCcur =  80. ; }
