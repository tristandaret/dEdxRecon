#/***************************************************************************
 * File: FuncFromTGraph.cxx
 * Project: dEdxRecon
 *
 * Brief: Implementation of a helper that converts a ROOT TGraph into a
 *        callable function-like object. Provides linear interpolation and
 *        a operator() suitable for use with TF1 wrappers.
 *
 * Contents: FuncFromTGraph constructor, Get_Y, operator() and Dump.
 *
 * Notes: Interface declared in FuncFromTGraph.h.
 ***************************************************************************/
...
FuncFromTGraph::FuncFromTGraph(TGraph *pTGraph) {
  ...
  //	std::cout << " m_StepX	" << std::setw(14) << std::setprecision(9) <<
  //m_StepX	<<
  // std::endl; 	std::cout << " m_X_Lowest	" << std::setw(14) <<
  // std::setprecision(9) << m_X_Lowest	<< std::endl; 	std::cout << "
  // m_X_Highest " << std::setw(14) << std::setprecision(9) << m_X_Highest <<
  // std::endl;
  //
  //	double* Y_pTGraph = p_TGraph ->GetX();
  //	std::cout
  //		<< " X_pTGraph[0] " << std::setw(14) << std::setprecision(9) <<
  //X_pTGraph[0]
  //		<< " Y_pTGraph[0] " << std::setw(14) << std::setprecision(9) <<
  //Y_pTGraph[0]
  //		<< std::endl;
  //	std::cout
  //		<< " X_pTGraph[1] " << std::setw(14) << std::setprecision(9) <<
  //X_pTGraph[1]
  //		<< " Y_pTGraph[1] " << std::setw(14) << std::setprecision(9) <<
  //Y_pTGraph[1]
  //		<< std::endl;
}
