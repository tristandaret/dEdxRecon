#/***************************************************************************
 * File: Uploader_MockUp_V2.cxx
 * Project: dEdxRecon
 *
 * Brief: Implementation of the mock-up uploader v2. Handles richer test
 *        datasets with many branches (pad waveforms, multi-dimensional
 *        vectors) and maps them into Event objects for reconstruction.
 *
 * Contents: constructor, Init, and GiveMe_Event implementation.
 *
 * Notes: See Uploader_MockUp_V2.h for the detailed member list used here.
 ***************************************************************************/
...
Event *Uploader_MockUp_V2::GiveMe_Event(const int &iEvent,
...
Event *Uploader_MockUp_V2::GiveMe_Event(const int &iEvent,
  ...
  if (beforeMerging != m_BeforeMerging)
    return 0;
  ...
  for (Int_t iCluster = 0; iCluster < iCluster_Max; iCluster++) {
    ...
    for (Int_t iPad = 0; iPad < iPad_Max; iPad++) {
      if (pad_charge->at(iCluster).at(iPad) < 0)
        continue;
      NberOfPads += 1;
