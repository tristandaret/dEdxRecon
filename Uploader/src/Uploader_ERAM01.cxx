#/***************************************************************************
 * File: Uploader_ERAM01.cxx
 * Project: dEdxRecon
 *
 * Brief: Implementation of the ERAM01 uploader. Reads ERAM01 formatted ROOT
 *        files and constructs Event objects with module/pad/waveform data.
 *
 * Contents: Uploader_ERAM01 constructor and GiveMe_Event implementation.
 *
 * Notes: See Uploader_ERAM01.h for the subclass interface.
 ***************************************************************************/
...
Uploader_ERAM01::Uploader_ERAM01(const std::string &SampleFile,
    ...
    : Uploader(SampleFile, pModel_ReadOutGeometry, pModel_Electronics,
               pModel_Charge1D) {
  std::cout << std::endl;
  std::cout << "	Uploader_ERAM01::Uploader_ERAM01 " << std::endl;
}

Uploader_ERAM01::~Uploader_ERAM01() {
  // std::cout << " In Uploader_ERAM01::~Uploader_ERAM01() " << std::endl;
}

Event *Uploader_ERAM01::GiveMe_Event(const int &iEvent,
...
Event *Uploader_ERAM01::GiveMe_Event(const int &iEvent,
  ...
  Double_t angle_yz = 0;
  Double_t angle_xy = 0;
  Int_t rob_clusters = 0;
  Double_t quality = 0;
  Double_t mom = 0;
  Double_t sina = 0;
  Double_t offset = 0;
  Int_t max_mult = 0;
  Double_t mean_mult = 0;
  Int_t multiplicity[70];
  Int_t charge[70];
  Double_t residual[70];
  Double_t residual_corr[70];
  Double_t dx[70][10];
  Double_t qfrac[70][10];
  Int_t time[70][10];
  Double_t clust_pos[70];
  Double_t clust_pos_err[70];
  Double_t track_pos[70];
  Int_t pad_charge[70][10];
  Int_t pad_time[70][10];
  Int_t wf_width[70][10];
  Int_t wf_fwhm[70][10];
  Int_t pad_x[70][10];
  Int_t pad_y[70][10];
  Int_t pad_wf_q[70][10][520];
  ...
  //
  Event *pEvent = new Event(ev, iEvent, p_Model_ReadOutGeometry,
  ...
  for (Int_t iCluster = 0; iCluster < iCluster_Max; iCluster++) {
    ...
    //	SetModule
    int ModuleNber_Cur = ModuleNber_Input; // This is to get a similar Code
                                           // Canevas than for other inputs
    if (ModuleNber_Input != -1 && ModuleNber_Cur != ModuleNber_Input)
      continue;

    Module *pModule = V_pModule[ModuleNber_Cur];

    if (charge[iCluster] < 0)
      continue;
    Cluster *pCluster = new Cluster(ev, iEvent, ModuleNber_Cur);

    // It seems that there are clusters (cluster charge >=0 but with no pad
    // (charge <0 for all pads) check and store the cluster only if there are
    // pads in it
    int NberOfPads = 0;
    for (Int_t iPad = 0; iPad < iPad_Max; iPad++) {
      ...
      Pad *pPad = new Pad(p_Model_ReadOutGeometry, p_Model_Electronics,
                          p_Model_Charge1D, "noName", ev, iEvent,
      ...
      for (Int_t iTimeBin = 0; iTimeBin < iTimeBin_Max; iTimeBin++) {
        ...
        if (ADCProper < -250)
          ADCProper = 0;
        if (iTimeBinProper < iTimeBin_Max)
