 ...
 * Project: dEdxRecon
 *
 * Brief: Base implementation for the Uploader class. Handles opening ROOT
 *        files, reading trees and extracting metadata such as the PRF
 *        function. Concrete uploaders implement GiveMe_Event().
 *
 * Contents: Uploader constructor/destructor and common helpers used by
 *           concrete uploader subclasses.
 *
 * Notes: See Uploader.h for the abstract interface.
 ***************************************************************************/
...
Uploader::Uploader(const std::string &sampleFile,
    ...
    : m_SampleFile(sampleFile), p_Model_ReadOutGeometry(pModel_ReadOutGeometry),
      p_Model_Electronics(pModel_Electronics),
      p_Model_Charge1D(pModel_Charge1D), m_PRF_exist(0) {
  std::cout << std::endl;
  std::cout << "Data file:      " << m_SampleFile << std::endl;

  p_TFile = TFile::Open(sampleFile.c_str(), "READ");
  if (!p_TFile) {
    std::cout << "Uploader::Uploader ERROR. Couldn't open root file: "
              << sampleFile << std::endl;
    abort();
  }
...
std::string Uploader::Get_SampleFile() { return m_SampleFile; }

int Uploader::Get_NberOfEvent() { return m_NberOfEvent; }

int Uploader::Get_PRF_exist() { return m_PRF_exist; }

double Uploader::Get_Norm() { return m_Norm; }

double Uploader::Get_a2() { return m_a2; }

double Uploader::Get_a4() { return m_a4; }

double Uploader::Get_b2() { return m_b2; }

double Uploader::Get_b4() { return m_b4; }
...
Model_Charge1D *Uploader::Get_Model_Charge1D() { return p_Model_Charge1D; }
