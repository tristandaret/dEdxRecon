...
Uploader::Uploader(
  ...
  if (!p_TFile) {
    std::cout << "Uploader::Uploader "
              <<" ERROR. " 
              << "Couldn't open root file : " << SampleFile
              <<std::endl;
    abort() ;
  }
...
//
  ...
  if (pTF1){
    ...
    if ( 
         std::string( pTF1->GetParName(0) ) == "Const"
...
//
  ...
  p_Model_Charge1D        = pModel_Charge1D        ;
...
Uploader::~Uploader()
{
//std::cout << " In Uploader::~Uploader() " << std::endl ;
//p_TFile->Close();
//delete p_TFile;
}
...
Model_ReadOutGeometry*  Uploader::Get_Model_ReadOutGeometry(){ return p_Model_ReadOutGeometry ;}
Model_Electronics    *  Uploader::Get_Model_Electronics    (){ return p_Model_Electronics     ;}
