...
TrackFitter::TrackFitter(const std::string &FitterName, const int &NberOfParameters)
{
   int ierr;
   double arg[1];
...
int TrackFitter::DoMinimisation()
{
   ...
   if (ierMIGRAD_N == 0) {
      ...
      if (ierMIGRAD_R1 == 0) {
         ...
         std::cout << " TrackFitter::DoMinimisation() Rescue 1 succeeded " << std::endl;
...
double TrackFitter::Chi2(double par[])
{
   return p_Track->Chi2(par);
}
