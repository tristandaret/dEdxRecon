...
float Reconstruction::ERAMMaps::RC(const int &position, const int &iX,
                                   const int &iY) {
  return fRC[position][iX][iY];
}

float Reconstruction::ERAMMaps::Gain(const int &position, const int &iX,
                                     const int &iY) {
...
void Reconstruction::ERAMMaps::setGain(const int &position, const int &iX,
                                       const int &iY, const float &gain) {
  fGain[position][iX][iY] = gain;
}

void Reconstruction::ERAMMaps::setRC(const int &position, const int &iX,
                                     const int &iY, const float &RC) {
...
void Reconstruction::ERAMMaps::setMeanGain(const int &position,
                                           const float &meanGain) {
  fmean_gain[position] = meanGain;
}
void Reconstruction::ERAMMaps::setMeanRC(const int &position,
                                         const float &meanRC) {
    ...
    for (int iY = 0; iY < 32; iY++) {
      for (int iX = 0; iX < 36; iX++) {
        ...
        if (gain == 0) {
          ...
          for (int i = 0; i < 4; i++)
            if (v_sides[i] !=
                0) { // additionnal step to discard empty neighbours
              gain += v_sides[i];
              n_sides++;
            }
          gain /= n_sides;
...
// Default constructor
Reconstruction::LUT::LUT(const int &transDiffCoeffB,
                         const int &transDiffCoeffnoB, const int &peakingTime) {
...
float Reconstruction::LUT::getRatio(const int &Dt, const int &RC,
                                    const float &drift, const float &impact,
                                    const float &angle) {
  // keep double
