...
void DrawOut_EventDisplay(Module *pModule, const std::string &OUTDIR,
  ...
  if (slope != 999 && intercept != 999) {
    pTF1_Track->SetParameters((intercept * 100 - 0.5) / 1.019,
                              slope * 1.128 / 1.019,
...
void DrawOut_ClusterWFDisplay(Cluster *pCluster, const std::string &OUTDIR,
  ...
  std::string OutputFile = aostringstream.str();
  std::string OutCWF =
      OUTDIR + TAG + "_CWF_Event_" + std::to_string(pCluster->Get_EventNber()) +
      "_Mod_" + std::to_string(pCluster->Get_ModuleNber()) + "_Cluster_" +
  ...
  for (int iP = 0; iP < NPads; iP++) { // Pads
    Pad *pPad = pCluster->Get_Pad(
        NPads - iP - 1); // Start by end of list to get most energetic pads
    TH1F *pTH1F = GiveMe_WaveFormDisplay(pPad, TAG);
  ...
  aostringstream_pTH1F_Sum << " Cluster "
  ...
  double ADCminWF =
      0; // Search in all pads the min and max value reached for ADCs
  double ADCmaxWF = 0;
  for (int iP = 0; iP < NPads; iP++) {
  ...
  for (int iP = 0; iP < NPads; iP++) {
    ...
    TLine *pTLineV_IN = new TLine(v_Pads[iPL]->Get_TMax(), ADCminWF,
                                  v_Pads[iPL]->Get_TMax(), ADCmaxWF);
    ...
    TLine *pTLineV_Lead = new TLine(v_Pads[0]->Get_TMax(), ADCminWF,
                                    v_Pads[0]->Get_TMax(), ADCmaxWF);
    ...
    if (Option >= 2) {
      ...
      if (v_Pads[iPL]->Get_FIT_Status() == 0) {
        ...
        ParabolaFunctionNG aParabolaFunctionNG;
        TF1 *pTF1 = new TF1("ParabolaFunctionNG", aParabolaFunctionNG, T_FitMin,
                            T_FitMax, 4);
...
void DrawOut_GWF(Event *pEvent, const int &ModuleNber,
  ...
  pTH1F_GWF->SetTitle(
      std::string(
          TAG + " Entry " + std::to_string(pEvent->Get_EntryNber()) +
          " Event " + std::to_string(pEvent->Get_EventNber()) + " Module " +
          std::to_string(
              pEvent->Get_Module_InArray(ModuleNber)->Get_ModuleNber()) +
  ...
  for (int iC = 0; iC < NClusters; iC++) {
    ...
    TH1F *pTH1F_WF_ETFcluster =
        ETF("pTH1F_WF_ETFcluster", 0, 510,
            pTH1F_cluster->GetMaximumBin() - PT / TB, 510, iC, PT, TB);
  ...
  TH1F *pTH1F_ETF =
      ETF("pTH1F_ETF", 0, 510, TmaxGWFsum - PT / TB, 510, 999, PT, TB);
  ...
  TLine *pTLine_TmaxGWFsum =
      new TLine(TmaxGWFsum - 1, Ymin, TmaxGWFsum - 1, Ymax);
  ...
  ptText->SetText(tmax, 0.4 * Ymax,
                  Form("#bar{dE/dx_{total}}	= %.0f ADCs ", maxGP));
...
void NewClusterDisplay(Event *pEvent, const std::string &OUTDIR,
  ...
  for (int iModule = 0; iModule < nModules; iModule++) {
    ...
    for (int iCluster = 0; iCluster < nClusters; iCluster++) {
      ...
      std::vector<TH1F *> vWF;
      TH1F *wfCluster = new TH1F(
          Form("wfCluster_%d_%d_%d_%d", pEvent->Get_EventNber(),
      ...
      vWF[0]->SetTitle(Form("Event %d, Module %d, Column %d",
                            pEvent->Get_EventNber(), pModule->Get_ModuleNber(),
                            pCluster->Get_LeadingPad()->Get_iX()));
      ...
      // Draw ETF
      TH1F *wfETF =
          ETF(Form("wfETF_%d_%d_%d_%d_%d", pEvent->Get_EventNber(),
                   pEvent->Get_EntryNber(), pModule->Get_ModuleNber(),
                   pCluster->Get_LeadingPad()->Get_iX(),
                   pCluster->Get_LeadingPad()->Get_iY()),
              0, 510, wfCluster->GetMaximumBin() - float(PT) / float(TB), 510,
              999, float(PT), float(TB));
      ...
      for (int i = 1; i <= wfDiff->GetNbinsX(); ++i) {
        double num =
            (wfCluster->GetBinContent(i) - wfETF->GetBinContent(i)) * 100;
      ...
      wfDiff->SetTitle(
          ";timebins;#frac{S_{sum} - S_{theo}}{S_{sum}^{max}} [%]");
...
void NewClusterDisplayMinimal(Event *pEvent, const std::string &OUTDIR,
  ...
  // Output file setup
  std::string outputFile = OUTDIR + "/" + TAG + "_ClusterWF_Evt" +
                           std::to_string(pEvent->Get_EventNber()) +
                           "_Minimal.pdf";
  ...
  for (int iModule = 0; iModule < nModules; iModule++) {
    ...
    for (int iCluster = 0; iCluster < nClusters; iCluster++) {
      ...
      std::vector<TH1F *> vWF;
      TH1F *wfCluster = new TH1F(
          Form("wfCluster_%d_%d_%d_%d_minimal", pEvent->Get_EventNber(),
               pModule->Get_ModuleNber(), pCluster->Get_LeadingPad()->Get_iX(),
               pCluster->Get_LeadingPad()->Get_iY()),
          "clusterWaveform", 510, 0, 510);
      int NPads = pCluster->Get_NberOfPads();
