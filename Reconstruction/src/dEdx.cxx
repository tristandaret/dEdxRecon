...
void Reconstruction::dEdx::Reconstruction() {
  ...
  // Log info
  std::cout << "dataFile:			  " << v_datafiles.back()
            << std::endl;
  ...
  // Take the reference as a track as long as a pad width, and normalize to 1 cm
  fAref = pcorrFunctionWF->Eval(XPADLENGTH) / XPADLENGTH * 10;
  ...
  if (tag.find("Theta") != std::string::npos)
    xmax = 2000;
  ph1f_WF = new TH1F("ph1f_WF",
                     "<dE/dx> with WF;<dE/dx> (ADC count);Number of events",
                     100, 0, xmax);
  ph1f_XP = new TH1F("ph1f_XP",
                     "<dE/dx> with XP;<dE/dx> (ADC count);Number of events",
                     100, 0, xmax);
  ph1f_GP1 = new TH1F("ph1f_GP1",
                      "<dE/dx> with GP1;<dE/dx> (ADC count);Number of events",
                      100, 0, xmax);
  ph1f_GP2 = new TH1F("ph1f_GP2",
                      "<dE/dx> with GP2;<dE/dx> (ADC count);Number of events",
                      100, 0, xmax);
  ph1f_GP3 = new TH1F("ph1f_GP3",
                      "<dE/dx> with GP3;<dE/dx> (ADC count);Number of events",
                      100, 0, xmax);
  ph1f_GP4 = new TH1F("ph1f_GP4",
                      "<dE/dx> with GP4;<dE/dx> (ADC count);Number of events",
                      100, 0, xmax);
  ph1f_GP5 = new TH1F("ph1f_GP5",
                      "<dE/dx> with GP5;<dE/dx> (ADC count);Number of events",
                      100, 0, xmax);
  ...
  std::string GP3paramPath =
      v_datafiles.back().substr(0, v_datafiles.back().length() - 5) +
      "_GP3param.root";
  ...
  if (ptfileGP3param && !ptfileGP3param->IsZombie()) {
    ...
    pth2f_paramGP3->SetDirectory(
        0); // avoids issue when writing in my output file
    ptfileGP3param->Close();
  } else {
    pth2f_paramGP3 =
        new TH2F("pth2f_paramGP3", "GP3 Parameters;Parameter;Value", 100, -1, 1,
                 100, 1, 2.5);
    ...
    ptf1_paramGP3 =
        new TF1("ptf1_paramGP3", "[0] + [1]*x*x + [2]*pow(x,4)", -0.6, 0.6);
  ...
  fpTree_dEdx->Branch("event_branch", "Reconstruction::RecoEvent",
                      &p_recoevent);
  ...
  for (iEvent = 0; iEvent < NEvents; iEvent++) {
    ...
    for (iMod = 0; iMod < NMod; iMod++) {
      ...
      for (iC = 0; iC < NClusters; iC++) {
        ...
        p_recocluster->ADCmax_base =
            pCluster->Get_Acluster(); // should not trust it
        ...
        for (iP = 0; iP < NPads; iP++) {
          ...
          case 1: // pad per pad gain correction
            p_recopad->gain = pERAMMaps->Gain(fERAMs_pos[fmodID], p_recopad->ix,
                                              p_recopad->iy);
          ...
          // Track computations
          local_params(pPad, pTrack, p_recopad->d, p_recopad->dd,
                       p_recopad->phi,
          ...
          // Compute drift distance for different time bin sizes and peaking
          // times
          ...
          if (iP == NPads - 1) {
            float halfdiag = fabs(
                sqrt(XPADLENGTH * XPADLENGTH + YPADLENGTH * YPADLENGTH) / 2);
            ...
            p_recocluster->LUTrhoLead = rhoGP4; // LUT ratio for leading pad
            AleadSafe =
                *std::max_element(waveform_pad.begin(), waveform_pad.end());
          }
          ...
          if (p_recopad->length * costheta > fminLength) {
            ...
            p_recopad->ratioFile =
                p_lut->getRatio(Dt, p_recopad->RC, driftDist,
                                fabs(p_recopad->d), fabs(p_recopad->phi));
            ...
            vCrossedWaveforms.push_back(
                hCrossedWF); // add after gain correction
          } else {
            p_recocluster->v_pads.push_back(p_recopad);
          }
        } // Pads
        ...
        if (p_recocluster->length * costheta > fminLength) {
          ...
          p_recocluster->charge = *std::max_element(waveform_cluster.begin(),
          ...
          // but a too short recocluster must not be stored, because it is not
          // used by WF
      ...
      p_recomodule->dEdxXPnoTrunc = ComputedEdxXP(
          v_mod_dEdxXP, v_mod_dE, v_mod_dx, p_recomodule->NCrossedPads, 1);
    ...
    p_recoevent->avg_pad_mult =
        (float)p_recoevent->NPads / p_recoevent->NClusters;
    ...
    p_recoevent->dEdxGP1 =
        ComputedEdxGP1(vClusterWaveforms, v_dEdxCluster, v_Aclus, v_Lclus,
                       p_recoevent->NClusters, falpha);
    p_recoevent->dEdxGP2 =
        ComputedEdxGP(p_recoevent->GWF, vClusterWaveforms, v_dEdxCluster,
                      v_Aclus, Levent, v_Lclus, p_recoevent->NClusters, falpha);
    p_recoevent->dEdxGP3 = ComputedEdxGP(
        p_recoevent->GWF, vClusterWaveforms, v_dEdxClusterGP3, v_AleadGP3,
        Levent, v_Lclus, p_recoevent->NClusters, falpha);
    p_recoevent->dEdxGP4 = ComputedEdxGP(
        p_recoevent->GWF, vClusterWaveforms, v_dEdxClusterGP4, v_AleadGP4,
        Levent, v_Lclus, p_recoevent->NClusters, falpha);
    p_recoevent->dEdxGP5 = ComputedEdxGP(
        p_recoevent->GWF, vCrossedWaveforms, v_evt_dEdxXP, v_evt_dE, Levent,
        v_evt_dx, p_recoevent->NCrossedPads, falpha);
    p_recoevent->dEdxWF =
        ComputedEdxWF(v_evt_dEdxWF, p_recoevent->NClusters, falpha);
    p_recoevent->dEdxXP = ComputedEdxXP(v_evt_dEdxXP, v_evt_dE, v_evt_dx,
                                        p_recoevent->NCrossedPads, falpha);
    p_recoevent->dEdxWFnoTrunc =
        ComputedEdxWF(v_evt_dEdxWF, p_recoevent->NClusters, 1);
    p_recoevent->dEdxXPnoTrunc = ComputedEdxXP(v_evt_dEdxXP, v_evt_dE, v_evt_dx,
                                               p_recoevent->NCrossedPads, 1);
...
float Reconstruction::dEdx::ComputedEdxWF(std::vector<float> v_dEdxWF,
  ...
  return std::accumulate(v_dEdxWF.begin(), v_dEdxWF.begin() + NClustersTrunc,
                         0.) /
         NClustersTrunc;
}
...
float Reconstruction::dEdx::ComputedEdxXP(const std::vector<float> &v_dEdx,
                                          ...
                                          const int &nCrossedPads,
                                          const float &alpha) {
  /// dEdx =	sum(dE)/Sum(dx) and not average(dE/dx) of each pad = > less
  ...
  std::iota(indices.begin(), indices.end(),
            0); // make list from 0 to v_dEdx.size()

  std::sort(indices.begin(), indices.end(),
            [&](int i, int j) { // sorting wrt v_dEdx
...
float Reconstruction::dEdx::ComputedEdxGP1(const std::vector<TH1F> &vClusWF,
  ...
  TH1F GWFtruncatedGP1(
      ("GWFtruncatedGP1" + std::to_string(iEvent) + std::to_string(iMod))
          .c_str(),
...
float Reconstruction::dEdx::ComputedEdxGP(
    const TH1F *GWF, const std::vector<TH1F> &vWF,
  ...
  for (int i = nElementsTruncated; i < nElements; ++i) {
    ...
    TH1F *etf =
        ETF("pTH1F_ETFtrunc_" + std::to_string(iEvent) + std::to_string(iMod),
...
TH1F *Reconstruction::dEdx::GetGigaWaveform(const std::vector<TH1F> &vClusWF) {
  // Prepare the GigaWaveform
  TH1F *GWF = new TH1F(
      ("getGWF" + std::to_string(iEvent) + std::to_string(iMod)).c_str(),
...
TH1F *Reconstruction::dEdx::GetTruncatedGigaWaveformGP1(
    const std::vector<TH1F> &vClusWF, const std::vector<float> &v_dEdx,
    const int &nClusters) {
  float nTruncatedClusters = int(floor(nClusters * 0.7));

  // Prepare the GigaWaveform
  TH1F *GWFtruncatedGP1 = new TH1F(
      ("getGWFtruncGP1" + std::to_string(iEvent) + std::to_string(iMod))
          .c_str(),
      "Truncated Giga Waveform GP1", 510, 0, 510);
