...
Selector::Selector(const std::string DefSelection) {
  ...
  if (DefSelection == "Sel_DESY21") {
    Set_DESY21_Event();
  }
  if (DefSelection == "Sel_DESY21_theta") {
    Set_DESY21theta_Event();
  }
  if (DefSelection == "Sel_CERN22") {
    Set_T2_CERN22_Event();
  }
  if (DefSelection == "TMC_CERN22_Event") {
    Set_TMC_CERN22_Event();
  }
...
void Selector::Set_DESY21theta_Event() {
  ...
  std::cout << std::endl;
  std::cout << "-------------->Predefined selection Sel_DESY21_theta"
            << std::endl;
  ListOfSelectionName.clear();
  //	Add_Selection( "Stage1"	);	// border clusters
  Add_Selection("Stage4");     // average pad multiplicity
...
void Selector::Set_TMC_CERN22_Event() {
  ...
  std::cout << std::endl;
  std::cout << "-------------->Predefined selection TMC_CERN22_Event"
            << std::endl;
  ListOfSelectionName.clear();
  //
  //	Add_Selection( "Stage1"	);	// border clusters
...
void Selector::Tell_Selection() {
  ...
  for (int iTem = 0; iTem < iTem_Max; iTem++) {
    if (ListOfSelectionName[iTem] == "Stage1") {
      Stage1_Def();
      std::cout << std::endl;
    }
    if (ListOfSelectionName[iTem] == "StageFinal") {
      StageFinal_Def();
      std::cout << std::endl;
    }
    if (ListOfSelectionName[iTem] == "Stage5") {
      Stage5_Def();
      std::cout << std::endl;
    }
    if (ListOfSelectionName[iTem] == "Stage2") {
      Stage2_Def();
      std::cout << std::endl;
    }
    if (ListOfSelectionName[iTem] == "Stage3") {
      Stage3_Def();
      std::cout << std::endl;
    }
    if (ListOfSelectionName[iTem] == "Stage6") {
      Stage6_Def();
      std::cout << std::endl;
    }
    if (ListOfSelectionName[iTem] == "Stage4") {
      Stage4_Def();
      std::cout << std::endl;
    }
  }
...
int Selector::NberOfSelections() { return ListOfSelectionName.size(); }
...
void Selector::Add_Selection(const std::string &SelectionName) {
  int IsOK = 0;
  if (SelectionName == "Stage1") {
    IsOK = 1;
  }
  if (SelectionName == "Stage2") {
    IsOK = 1;
  }
  if (SelectionName == "Stage3") {
    IsOK = 1;
  }
  if (SelectionName == "Stage4") {
    IsOK = 1;
  }
  if (SelectionName == "Stage5") {
    IsOK = 1;
  }
  if (SelectionName == "Stage6") {
    IsOK = 1;
  }
  if (SelectionName == "StageFinal") {
    IsOK = 1;
  }

  if (IsOK == 0) {
    std::cout << "Selector::Add_Selection "
              << " unknown selection " << SelectionName << std::endl;
    abort();
  }
...
void Selector::Apply_ASelection(Sample &aSample, const int &ModuleNber,
                                const int &iTem) {
  ...
  if (SelectionName == "Stage1") {
    Stage1(aSample, ModuleNber);
  }
  if (SelectionName == "StageFinal") {
    StageFinal(aSample, ModuleNber);
  }
  if (SelectionName == "Stage5") {
    Stage5(aSample, ModuleNber);
  }
  if (SelectionName == "Stage2") {
    Stage2(aSample, ModuleNber);
  }
  if (SelectionName == "Stage3") {
    Stage3(aSample, ModuleNber);
  }
  if (SelectionName == "Stage6") {
    Stage6(aSample, ModuleNber);
  }
  if (SelectionName == "Stage4") {
    Stage4(aSample, ModuleNber);
  }

  SetStat_After(aSample, ModuleNber, iTem);
}

void Selector::SetStat_Before(Sample &aSample, const int &ModuleNber,
                              const int &iTem) {
...
void Selector::SetStat_After(Sample &aSample, const int &ModuleNber,
                             const int &iTem) {
  int NberOfEvents = aSample.Get_NberOfEvents();
  int NberOfValidEvents = 0;
  int NberOfClusters = 0;
...
void Selector::Apply_ASelection(Event *pEvent, const int &ModuleNber,
                                const int &iTem) {
  std::string SelectionName = ListOfSelectionName[iTem];
...
void Selector::PrintStat() {
  ...
  std::cout << std::setw(5) << std::setprecision(0) << "#Cut" << std::setw(8)
            << std::setprecision(0) << "SelName" << std::setw(9)
            << std::setprecision(0) << "EvBef" << std::setw(9)
            << std::setprecision(0) << "EvAfter"
            << " (" << std::setw(8) << "wrtPrev"
            << ") "
            << " (" << std::setw(8) << "wrtStart"
            << ") " << std::setw(9) << std::setprecision(0) << "MdBef"
            << std::setw(9) << std::setprecision(0) << "MdAfter"
            << " (" << std::setw(8) << "wrtPrev"
            << ") "
            << " (" << std::setw(8) << "wrtStart"
            << ") " << std::setw(9) << std::setprecision(0) << "ClBef"
            << std::setw(9) << std::setprecision(0) << "ClAfter"
            << " (" << std::setw(8) << "wrtPrev"
            << ") "
            << " (" << std::setw(8) << "wrtStart"
            << ") " << std::setw(9) << std::setprecision(0) << "PdBef"
            << std::setw(9) << std::setprecision(0) << "PdAfter"
            << " (" << std::setw(8) << "wrtPrev"
            << ") "
            << " (" << std::setw(8) << "wrtStart"
            << ") " << std::endl;
  ...
  for (int iTem = 0; iTem < iTem_Max; iTem++) {
    std::cout
        ...
        << std::setprecision(0) << ListOfNberOfEvents_Before[iTem]
        << std::setw(9) << std::setprecision(0)
        << ListOfNberOfEvents_After[iTem] << " (" << std::setw(8)
        << std::setprecision(2)
        << 100. *
               (ListOfNberOfEvents_After[iTem] -
                ListOfNberOfEvents_Before[iTem]) /
               ListOfNberOfEvents_Before[iTem]
        << ") "
        << " (" << std::setw(8) << std::setprecision(2)
        << 100. *
               (ListOfNberOfEvents_After[iTem] - ListOfNberOfEvents_Before[0]) /
               ListOfNberOfEvents_Before[0]
        << ") " << std::setw(9) << std::setprecision(0)
        << ListOfNberOfModules_Before[iTem] << std::setw(9)
        << std::setprecision(0) << ListOfNberOfModules_After[iTem] << " ("
        << std::setw(8) << std::setprecision(2)
        << 100. *
               (ListOfNberOfModules_After[iTem] -
        ...
        << 100. *
               ...
               ListOfNberOfModules_Before[0]
        << ") " << std::setw(9) << std::setprecision(0)
        << ListOfNberOfClusters_Before[iTem] << std::setw(9)
        << std::setprecision(0) << ListOfNberOfClusters_After[iTem] << " ("
        << std::setw(8) << std::setprecision(2)
        << 100. *
               (ListOfNberOfClusters_After[iTem] -
        ...
        << 100. *
               ...
               ListOfNberOfClusters_Before[0]
        << ") " << std::setw(9) << std::setprecision(0)
        << ListOfNberOfPads_Before[iTem] << std::setw(9) << std::setprecision(0)
        << ListOfNberOfPads_After[iTem] << " (" << std::setw(8)
        << std::setprecision(2)
        << 100. *
               (ListOfNberOfPads_After[iTem] - ListOfNberOfPads_Before[iTem]) /
  ...
  std::cout << std::setw(13) << std::setprecision(0) << "Summary Sel"
            << std::setw(9) << std::setprecision(0)
            << ListOfNberOfEvents_Before[0] << std::setw(9)
            << std::setprecision(0) << ListOfNberOfEvents_After[iTem_Max - 1]
            << std::setw(12) << " "
            << " (" << std::setw(8) << std::setprecision(2)
            << 100. *
                   (ListOfNberOfEvents_After[iTem_Max - 1] -
                    ListOfNberOfEvents_Before[0]) /
                   ListOfNberOfEvents_Before[0]
            << ") " << std::setw(9) << std::setprecision(0)
            << ListOfNberOfModules_Before[0] << std::setw(9)
            << std::setprecision(0) << ListOfNberOfModules_After[iTem_Max - 1]
            << std::setw(12) << " "
            << " (" << std::setw(8) << std::setprecision(2)
            << 100. *
                   (ListOfNberOfModules_After[iTem_Max - 1] -
                    ListOfNberOfModules_Before[0]) /
                   ListOfNberOfModules_Before[0]
            << ") " << std::setw(9) << std::setprecision(0)
            << ListOfNberOfClusters_Before[0] << std::setw(9)
            << std::setprecision(0) << ListOfNberOfClusters_After[iTem_Max - 1]
            << std::setw(12) << " "
            << " (" << std::setw(8) << std::setprecision(2)
            << 100. *
                   (ListOfNberOfClusters_After[iTem_Max - 1] -
                    ListOfNberOfClusters_Before[0]) /
                   ListOfNberOfClusters_Before[0]
            << ") " << std::setw(9) << std::setprecision(0)
            << ListOfNberOfPads_Before[0] << std::setw(9)
            << std::setprecision(0) << ListOfNberOfPads_After[iTem_Max - 1]
            << std::setw(12) << " "
            << " (" << std::setw(8) << std::setprecision(2)
            << 100. *
                   (ListOfNberOfPads_After[iTem_Max - 1] -
                    ListOfNberOfPads_Before[0]) /
                   ListOfNberOfPads_Before[0]
            << ") " << std::endl;
}
...
void Selector::Stage1(Event *pEvent, const int &ModuleNber) {
  ...
  for (int iC = 0; iC < NClusters; iC++) {
    ...
    if (iXCluster == iXFirst || iXCluster == iXLast)
      pCluster->Invalidate();
  }
}
...
void Selector::Set_Cut_Stage2_EventBased(double Cut_Stage2_EventBased) {
  m_Cut_Stage2_EventBased = Cut_Stage2_EventBased;
}
void Selector::Stage2_Def() {
  std::cout << "Apply selection 2 -> Remove clusters out of time (Event Based) "
            << std::endl;
  std::cout
      << "|TLeading -TLeadingMeanOverEvent| <= m_Cut_Stage2_EventBased (= "
...
void Selector::Set_Cut_Stage3_TLow(double Cut_Stage3_TLow) {
  m_Cut_Stage3_TLow = Cut_Stage3_TLow;
}
void Selector::Set_Cut_Stage3_THigh(double Cut_Stage3_THigh) {
  m_Cut_Stage3_THigh = Cut_Stage3_THigh;
}
void Selector::Stage3_Def() {
  std::cout << "Apply selection 3 -> Remove clusters out of time	"
            << std::endl;
  std::cout << "m_Cut_Stage3_TLow <= TLeading <= m_Cut_Stage3_THigh "
            << std::endl;
  std::cout << "m_Cut_Stage3_TLow	= " << std::setw(4)
            << std::setprecision(0) << m_Cut_Stage3_TLow << std::endl;
  std::cout << "m_Cut_Stage3_THigh	= " << std::setw(4)
            << std::setprecision(0) << m_Cut_Stage3_THigh << std::endl;
}
...
void Selector::Stage3(Event *pEvent, const int &ModuleNber) {
  ...
  for (int iC = 0; iC < NClusters; iC++) {
    ...
    if (m_Cut_Stage3_TLow > TLeading or TLeading > m_Cut_Stage3_THigh)
      pCluster->Invalidate();
  }
}
...
void Selector::Set_Cut_Stage4_APM_Low(double Cut_Stage4_APM_Low) {
  m_Cut_Stage4_APM_Low = Cut_Stage4_APM_Low;
}
double Selector::Get_Cut_Stage4_APM_High() { return m_Cut_Stage4_APM_High; }
void Selector::Set_Cut_Stage4_APM_High(double Cut_Stage4_APM_High) {
  m_Cut_Stage4_APM_High = Cut_Stage4_APM_High;
}
void Selector::Stage4_Def() {
  std::cout << "Apply selection 4 -> Remove Events with an average pad "
               "multiplicity too "
               "high or too low "
            << std::endl;

  std::cout << "Pad Multiplicity >	m_Cut_Stage4_APM_Low	(= "
            << std::setw(4) << std::setprecision(1) << m_Cut_Stage4_APM_Low
            << ")" << std::endl;
  std::cout << "Pad Multiplicity <= m_Cut_Stage4_APM_High (= " << std::setw(4)
            << std::setprecision(1) << m_Cut_Stage4_APM_High << ")"
            << std::endl;
}
...
void Selector::Stage4(Event *pEvent, const int &ModuleNber) {
  ...
  if (PadMulti <= m_Cut_Stage4_APM_Low or PadMulti > m_Cut_Stage4_APM_High)
...
void Selector::Set_Cut_Stage5_Npads_Hig(double Cut_Stage5_Npads_Hig) {
  m_Cut_Stage5_Npads_Hig = Cut_Stage5_Npads_Hig;
}
void Selector::Stage5_Def() {
  std::cout << "Apply selection 5 -> Remove clusters with too many pads "
            << std::endl;
  std::cout << "Npads <= m_Cut_Stage5_Npads_Hig (= " << std::setw(3)
            << std::setprecision(0) << m_Cut_Stage5_Npads_Hig << ")"
            << std::endl;
}
...
void Selector::Stage5(Event *pEvent, const int &ModuleNber) {
  ...
  for (int iC = 0; iC < NClusters; iC++) {
    Cluster *pCluster = ClusterSet[iC];
    int NPads = pCluster->Get_NberOfPads();
    if (NPads > m_Cut_Stage5_Npads_Hig)
      pCluster->Invalidate();
  }
}
...
void Selector::Set_Cut_Stage6_Amax_Low(double Cut_Stage6_Amax_Low) {
  m_Cut_Stage6_Amax_Low = Cut_Stage6_Amax_Low;
}
double Selector::Get_Cut_Stage6_Amax_Hig() { return m_Cut_Stage6_Amax_Hig; }
void Selector::Set_Cut_Stage6_Amax_Hig(double Cut_Stage6_Amax_Hig) {
  m_Cut_Stage6_Amax_Hig = Cut_Stage6_Amax_Hig;
}
void Selector::Stage6_Def() {
  std::cout << "Apply selection 6 -> Remove Clusters with a too high APad "
            << std::endl;
  std::cout << "APad >= m_Cut_Stage6_Amax_Low (= " << std::setw(5)
            << std::setprecision(0) << m_Cut_Stage6_Amax_Low << ")"
            << std::endl;
  std::cout << "APad <= m_Cut_Stage6_Amax_Hig (= " << std::setw(5)
            << std::setprecision(0) << m_Cut_Stage6_Amax_Hig << ")"
            << std::endl;
}
...
double Selector::Get_Cut_StageFinal_NCluster_Low() {
  return m_Cut_StageFinal_NCluster_Low;
}
void Selector::Set_Cut_StageFinal_NCluster_Low(
    double Cut_StageFinal_NCluster_Low) {
  m_Cut_StageFinal_NCluster_Low = Cut_StageFinal_NCluster_Low;
}
void Selector::StageFinal_Def() {
  ...
  std::cout << "NClusters >= m_Cut_StageFinal_NCluster_Low (=" << std::setw(5)
            << std::setprecision(0) << m_Cut_StageFinal_NCluster_Low << ")"
            << std::endl;
}
