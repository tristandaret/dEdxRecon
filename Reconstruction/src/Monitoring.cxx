...
void Reconstruction::Monitoring()
{
   ...
   if (prototype) {
      ...
      for (int iFile : part_arr) {
         if (iFile == 0) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_e+_0p5GeV_iter0.root");
            v_tags.push_back("e+_0p5GeV");
         }
         if (iFile == 1) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_e+_1GeV_iter0.root");
            v_tags.push_back("e+_1GeV");
         }
         if (iFile == 2) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_p_1GeV_iter0.root");
            v_tags.push_back("p_1GeV");
         }
         if (iFile == 3) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_mu-_1GeV_iter0.root");
            v_tags.push_back("mu-_1GeV");
         }
         if (iFile == 4) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_mu+_0p75GeV_iter0.root");
            v_tags.push_back("mu+_0p75GeV");
         }
         if (iFile == 5) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_mu+_1GeV_iter0.root");
            v_tags.push_back("mu+_1GeV");
         ...
         if (iFile == 6) {
            v_datafiles.push_back(data_scanpath + "ERAM18_350V_412ns_pi+_0p5GeV_iter0.root");
            v_tags.push_back("pi+_0p5GeV");
         }
      }
   }
   ...
   if (CERN_Escan) {
      ...
      v_valstr.insert(v_valstr.end(),
                      {"e+_0p5GeV", "e+_0p75GeV", "e+_1GeV", "e+_1p25GeV", "e+_1p5GeV", "mu+_0p75GeV", "mu+_1GeV",
                       "mu+_1p5GeV", "pi+_0p75GeV", "pi+_1p25GeV", "pi+_1p5GeV", "p_1GeV", "p_1p25GeV", "p_1p5GeV"});

      std::vector<std::string> v_suffix = {
         "_25V_z415_y2pad_iter0.root",     "_25V_z415_y2pad_iter0.root",   "_25V_z415p1_y2pad_iter0.root",
         "_25V_z415p1_y2pad_1_iter0.root", "_25V_z415p1_y2pad_iter0.root", "_25V_z415_y2pad_iter0.root",
         "_25V_z415p1_y2pad_iter0.root",   "_25V_z415p1_y2pad_iter0.root", "_25V_z415_y2pad_iter0.root",
         "_25V_z415p1_y2pad_2_iter0.root", "_25V_z415p1_y2pad_iter0.root", "_25V_z415p1_y2pad_iter0.root",
   ...
   if (DESY_drift) {
      ...
      for (int iPT : PT_arr) {
         ...
         Settings("DESY21", "Drift", Form("Drift_PT%i", PT), Form("%i ns peak", PT), "Drift distance (mm)", 1, 0, PT,
                  310, 450, 50);

         for (int iz = 0; iz < (int)std::size(v_valstr); iz++) {
            const char *z = v_valstr[iz].c_str();
            driftDist = v_valint[iz];
            v_datafiles.push_back(data_scanpath +
                                  Form("Drift_scan_PT%i/z_360_275_%i_02T_26_%s_iter9.root", iPT, iPT, z));
            v_tags.push_back(Form("%s_%s_Z%s", testbeam.c_str(), scan.c_str(), z));
   ...
   if (DESY_row) {
      ...
      for (int y = 0; y < (int)std::size(v_valstr); y++) {
         v_datafiles.push_back(data_scanpath + Form("Row_scan/Y%s_Z0_iter9.root", v_valstr[y].c_str()));
         v_tags.push_back(Form("%s_%s_Y%s", testbeam.c_str(), scan.c_str(), v_valstr[y].c_str()));
         DefaultAnalysis();
      }
      if (Draw_DESY21SingleScan)
         DrawSingleScan();
   ...
   if (DESY_phi) {
      ...
      for (int iz = 0; iz < (int)v_valdrift.size(); iz++) {
         ...
         Settings("DESY21", "Phi", Form("Phi_Z%s", z), Form("%2i cm drift", v_valdrift[iz] / 10),
                  "Track angle #varphi (#circ)", 1, 0, 200, 310, v_valdrift[iz], 40);

         for (int ifile = 0; ifile < (int)std::size(v_valint); ifile++) {
            // for (int ifile = 5; ifile < 6; ifile++){
            int phi = v_valint[ifile];
            if (ifile == 0) {
               v_datafiles.push_back(data_scanpath + Form("Drift_scan_PT200/z_360_275_200_02T_26_%s_iter9.root", z));
               v_tags.push_back(Form("%s_%s_Phi%i", testbeam.c_str(), scan.c_str(), phi));
            } else if (ifile < 4) {
               v_datafiles.push_back(data_scanpath + Form("Phi_scan_z%s/phi_200_%i_z%s_ym60_iter9.root", z, phi, z));
               v_tags.push_back(Form("%s_%s_Phi%i", testbeam.c_str(), scan.c_str(), phi));
            } else {
               v_datafiles.push_back(data_scanpath +
                                     Form("Phi_scan_z%s/phi_200_%i_z%s_ym60_diag_iter9.root", z, phi, z));
               v_tags.push_back(Form("%s_%s_Phi%i_diag", testbeam.c_str(), scan.c_str(), phi));
            }
   ...
   if (DESY_theta) {
      ...
      for (int ifile = 0; ifile < (int)std::size(v_valint); ifile++) {
         ...
         v_datafiles.push_back(data_scanpath + Form("Theta_scan/theta_%s_02T_z460_ym60_iter9.root", theta));
         v_tags.push_back(Form("%s_%s_%s", testbeam.c_str(), scan.c_str(), theta));
...
void Reconstruction::Settings(const std::string &testbeam_name, const std::string &metascan_name,
                              ...
                              const int &peaking_time, const int &diffusion, const int &drift_dist, const int &timbin)
{
   ...
   if (metascan != "")
      drawout_scanpath = drawout_metascanpath + testbeam + "_" + scan + "/";
   else
      drawout_scanpath = drawout_path + testbeam + "_" + scan + "/";
   ...
   if (dedx)
      p_lut = new LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root", Dt, PT));
}
...
//	moduleCase =	0;
//	PT = 200; Dt = 310;; TB = 40;
//	if (control or dedx) p_lut = new LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root",
// Dt, PT)); 	std::string B_arr[] = {"0", "04", "06"}; 	std::string Z_arr[] = {"60", "460", "860"}; 	int v_valint[] =
// {150, 550, 950}; 	for(int j = 0; j < 3; j++){ 		const char* B = B_arr[j].c_str(); 		for(int i = 0; i < 3; i++){
// const char* z = Z_arr[i].c_str(); 		driftDist = v_valint[i];
//		// v_datafiles.push_back(Form			("../Data/Data_DESY21/ExB_scan/ExB_360_412ns_B02_ym10_z%s_iter9.root", z));
// v_tags.push_back(Form("DESY21_ExB02_Z%s", z)); v_prtcles.push_back(Form("electron_ExB02_Z%s", z);
//		v_datafiles.push_back(Form			("../Data/Data_DESY21/ExB_scan/ExB_360_phim3_200ns_B%s_ym60_z%s_iter0.root", B,
// z)); v_tags.push_back(Form("DESY21_ExB%s_Z%s", B, z)); v_prtcles.push_back(Form("electron_ExB%s_Z%s", B, z));
// if(control or dedx) p_uploader = GiveMe_Uploader (intUploader, v_datafiles.back()); 		if (control)
// Control (drawout_scanpath, v_tags.back(), comment, v_datafiles.back(), selectionSet, p_uploader, moduleCase, 0, PT,
// TB, v_prtcles.back()); 		if (Draw_control)			DrawOut_Control (drawout_scanpath, v_tags.back(), comment,
// selectionSet, 1); 		if (dedx)				p_dEdx->Reconstruction(); 		if (Draw_dEdx)			DrawOut_Methods
//(drawout_scanpath, v_tags.back(), comment, 1, v_prtcles.back()); 		if (Draw_Checks)				DrawOut_Checks
//(drawout_scanpath, v_datafiles.back(), v_tags.back(), comment);
//		}
//	}
//	}
...
//	if (control or dedx) p_lut = new LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root",
// Dt, PT)); 	int phi_val[] = {0, 10, 20, 30, 40, 45}; 	int NFiles = 6; 	drawout_scanpath = drawout_path +
// "/DESY19_phi/"; 	MakeMyDir(drawout_scanpath); 	for (int ifile = 3; ifile < NFiles; ifile++){ 		int phi =
// phi_val[ifile];
//		if(ifile < 3) { v_datafiles.push_back(Form("../Data_DESY19/Phi_scan/phi_412_%i_iter0.root", phi));
// v_tags.push_back(Form("DESY19_phi%i", phi));		v_prtcles.push_back(Form("electron_phi%i", phi));} 		else {
// v_datafiles.push_back(Form("../Data_DESY19/Phi_scan/phi_412_%i_diag_iter0.root", phi));
// v_tags.push_back(Form("DESY19_phi%i_diag", phi)); v_prtcles.push_back(Form("electron_phi%i_diag", phi));}
// if(control or dedx) p_uploader = GiveMe_Uploader (intUploader, v_datafiles.back()); 		if (control)		Control
//(drawout_scanpath, v_tags.back(), comment, v_datafiles.back(), selectionSet, p_uploader, moduleCase, 0, PT, TB,
// v_prtcles.back()); 		if (Draw_control)	DrawOut_Control			(drawout_scanpath, v_tags.back(), comment,
// selectionSet, 1); 		if (dedx)			p_dEdx->Reconstruction(); 		if (Draw_Checks)	DrawOut_Checks
// (drawout_scanpath, v_datafiles.back(), v_tags.back(), comment); 		if (Draw_dEdx)	DrawOut_Methods
// (drawout_scanpath, v_tags.back(), comment, 1, v_prtcles.back()); 		delete p_uploader;
//	}
//	}
...
//		// v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_MD_RC100_v2_iter4.root", zDrift+1));
// v_tags.push_back(Form("CERN23_MC_z%i00", zDrift+1)); v_prtcles.push_back(Form("MC %i0cm", zDrift+1);
//		v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_NoOpt_iter4.root", zDrift+1));
// v_tags.push_back(Form("CERN23_MC_z%i00", zDrift+1)); v_prtcles.push_back(Form("MC %i0cm NoOpt", zDrift+1));
//		// v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_RC100_iter4.root", zDrift+1));
// v_tags.push_back(Form("CERN23_MC_z%i00_old", zDrift+1)); v_prtcles.push_back(Form("MC %i0cm old", zDrift+1);
//		if(control or dedx) p_uploader = GiveMe_Uploader (intUploader, v_datafiles.back());
//		if (control)		Control		(drawout_scanpath, v_tags.back(), comment, v_datafiles.back(), selectionSet,
// p_uploader, moduleCase, 0, PT, TB, v_prtcles.back()); 		if (Draw_control)	DrawOut_Control
// (drawout_scanpath,
// v_tags.back(), comment, selectionSet, 1); 		if (dedx)			p_dEdx->Reconstruction(); 		if (Draw_Checks)
// DrawOut_Checks (drawout_scanpath, v_datafiles.back(), v_tags.back(), comment); 		if (Draw_dEdx)	DrawOut_Methods
//(drawout_scanpath, v_tags.back(), comment, 1, v_prtcles.back());
//	}
//	}
...
//		MakeMyDir(drawout_scanpath);
//	if (control or dedx) p_lut = new LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root",
// Dt, PT)); 		int NFiles = 9; 		for (int zDrift = -1; zDrift < NFiles; zDrift++){ 		if(zDrift == -1)
//{v_datafiles.push_back(data_scanpath + Form("zscan_PT%i_139V/z_360_139_%i_02T_26_m40_iter0.root", PT, PT));
// v_tags.push_back(Form("DESY21_zm40_PT%i", PT));			} 		else {v_datafiles.push_back(data_scanpath +
// Form("zscan_PT%i_139V/z_360_139_%i_02T_26_%i60_iter0.root", PT, PT, zDrift));
// v_tags.push_back(Form("DESY21_z%i60_PT%i", zDrift, PT)); v_prtcles.push_back(Form("electron_z%i60", zDrift));}
//		if(control or dedx) p_uploader = GiveMe_Uploader (intUploader, v_datafiles.back());
//		if (control)		Control		(drawout_scanpath, v_tags.back(), comment, v_datafiles.back(), selectionSet,
// p_uploader, moduleCase, 0, PT, TB, v_prtcles.back()); 		if (Draw_control)	DrawOut_Control
// (drawout_scanpath,
// v_tags.back(), comment, selectionSet, 1); 		if (dedx)			p_dEdx->Reconstruction(); 		if (Draw_dEdx)
// DrawOut_Methods (drawout_scanpath, v_tags.back(), comment, 1, v_prtcles.back());
//		}
//		if(Draw_Resolution)	DrawOut_Zscan	(drawout_path + Form("/DESY21_Drift/DESY21_Drift_PT%i_139V", PT), comment,
// PT);
//	}
//	}