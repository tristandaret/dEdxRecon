...
void Reconstruction::Monitoring() {
  ...
  if (prototype) {
    Settings("CERN22", "", "Prototype", "", "Energy (GeV)", 2, -1, 412, 350,
             415, 40);
    ...
    for (int iFile : part_arr) {
      ...
      if (iFile == 1) {
        v_datafiles.push_back(dataScanPath +
                              "ERAM18_350V_412ns_e+_1GeV_iter0.root");
        vTags.push_back("e+_1GeV");
      }
      if (iFile == 2) {
        v_datafiles.push_back(dataScanPath +
                              "ERAM18_350V_412ns_p_1GeV_iter0.root");
  ...
  if (CERN_Escan) {
    Dt = DtwithoutBhere;
    Settings("CERN22", "", "Energy", "", "Energy (GeV)", 4, -1, 412, Dt, 415,
             40);
    vScanVals.insert(vScanVals.end(), {0.5, 0.75, 1, 1.25, 1.5, 0.75, 1, 1.5,
                                       0.75, 1.25, 1.5, 1, 1.25, 1.5});
    vScanLabels.insert(vScanLabels.end(),
                       {"e+_0p5GeV", "e+_0p75GeV", "e+_1GeV", "e+_1p25GeV",
                        "e+_1p5GeV", "mu+_0p75GeV", "mu+_1GeV", "mu+_1p5GeV",
                        "pi+_0p75GeV", "pi+_1p25GeV", "pi+_1p5GeV", "p_1GeV",
                        "p_1p25GeV", "p_1p5GeV"});
  ...
  if (CERN_drift) {
    Settings("CERN22", "", "Drift", "", "Drift distance (mm)", 3, -1, 412, 350,
             415, 40);
    ...
    for (int iz = 0; iz < NFiles; iz++) {
      ...
      v_datafiles.push_back(dataScanPath +
                            "All_ERAMS_350V_412ns_e+_0p5GeV_25V_z" + z +
                            "_iter4.root");
  ...
  if (DESY_drift) {
    ...
    vScanLabels.insert(vScanLabels.end(), {"m40", "060", "160", "260", "360",
                                           "460", "560", "660", "760", "860"});
    for (int iPT : PT_arr) {
      PT = iPT; // PT is a shared variable in the namespace so it's important to
                // update it here
      ...
      for (int iz = 0; iz < (int)std::size(vScanLabels); iz++) {
        ...
        v_datafiles.push_back(
            dataScanPath +
            Form("Drift_scan_PT%i/z_360_275_%i_02T_26_%s_iter9.root", iPT, iPT,
                 z));
        vTags.push_back(
  ...
  if (DESY_row) {
    Dt = DtwithBhere;
    Settings("DESY21", "", "Row", "", "Vertical beam position (mm)", 1, 0, 412,
             Dt, 90, 50);
    vScanVals.insert(vScanVals.end(),
                     {-140, -120, -100, -80, -60, -40, 0, 20, 40, 60, 80});
    vScanLabels.insert(vScanLabels.end(), {"m140", "m120", "m100", "m80", "m60",
                                           "m40", "0", "20", "40", "60", "80"});

    for (int y = 0; y < (int)std::size(vScanLabels); y++) {
      v_datafiles.push_back(dataScanPath + Form("Row_scan/Y%s_Z0_iter9.root",
                                                vScanLabels[y].c_str()));
  ...
  if (DESY_phi) {
    ...
    for (int iz = 0; iz < (int)v_valdrift.size(); iz++) {
      ...
      Settings("DESY21", "Phi", Form("Phi_Z%s", z),
               Form("%2i cm", v_valdrift[iz] / 10),
               "Track angle #varphi (#circ)", 1, 0, 200, Dt, v_valdrift[iz],
               40);
      ...
      for (int ifile = 0; ifile < (int)std::size(vScanVals); ifile++) {
        ...
        if (ifile == 0) {
          ...
          vTags.push_back(Form("%s_%s_Phi%i_Dt%d", testbeam.c_str(),
                               scanName.c_str(), phi, Dt));
        } else if (ifile < 4) {
          v_datafiles.push_back(
              dataScanPath + Form("Phi_scan_z%s/phi_200_%i_z%s_ym60_iter9.root",
                                  z, phireal, z));
          vTags.push_back(Form("%s_%s_Phi%i_Dt%d", testbeam.c_str(),
          ...
          v_datafiles.push_back(
              dataScanPath +
              Form("Phi_scan_z%s/phi_200_%i_z%s_ym60_diag_iter9.root", z,
                   phireal, z));
  ...
  if (DESY_theta) {
    Dt = DtwithBhere;
    Settings("DESY21", "", "Theta", "", "#theta angle (#circ)", 1, 0, 200, Dt,
             350, 40);
    ...
    for (int ifile = 0; ifile < (int)std::size(vScanVals); ifile++) {
      ...
      v_datafiles.push_back(
          dataScanPath +
          Form("Theta_scan/theta_%s_02T_z460_ym60_iter9.root", theta));
      vTags.push_back(
...
void Reconstruction::Correction(const int &corrRC, const int &corrGain,
                                const int &corrWF, const int &corrDrift,
  ...
  if (corrWF ==
      4) // Use my (40, m40) corr func shifted down by 100 ADCs for all
...
void Reconstruction::Settings(const std::string &testbeam_name,
                              ...
                              const std::string &scanspec,
                              const std::string &runvarstr_name,
                              const int &uploader, const int &modules,
                              const int &peaking_time, const int &diffusion,
...
// DtB, PT)); 	std::string B_arr[] = {"0", "04", "06"}; 	std::string
// Z_arr[] = {"60", "460", "860"}; 	int vScanVals[] = {150, 550, 950};
// for(int j = 0; j < 3; j++){ const char* B = B_arr[j].c_str();
// for(int i = 0; i < 3; i++){ const char* z = Z_arr[i].c_str();
// driftDist = vScanVals[i];
...
// v_datafiles.back(), selectionSet, p_uploader, moduleCase, 0, PT, TB,
// v_prtcles.back()); 		if (Draw_control) DrawOut_Control
// (drawoutScanPath, vTags.back(), comment, selectionSet, 1); 		if
// (dedx)				p_dEdx->Reconstruction();
// if (Draw_dEdx)			DrawOut_Methods
//(drawoutScanPath, vTags.back(), comment, 1, v_prtcles.back()); if
//(Draw_Checks)
...
//	if (control or dedx) p_lut = new
// LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root",
// DtB, PT)); 	int phi_val[] = {0, 10, 20, 30, 40, 45}; 	int NFiles = 6;
// drawoutScanPath = drawoutPath +
// "/DESY19_phi/"; 	MakeMyDir(drawoutScanPath); 	for (int ifile = 3;
// ifile < NFiles; ifile++){ 		int phi = phi_val[ifile];
//		if(ifile < 3) {
// v_datafiles.push_back(Form("../Data_DESY19/Phi_scan/phi_412_%i_iter0.root",
// phi)); vTags.push_back(Form("DESY19_phi%i", phi));
// v_prtcles.push_back(Form("electron_phi%i", phi));} 		else {
// v_datafiles.push_back(Form("../Data_DESY19/Phi_scan/phi_412_%i_diag_iter0.root",
// phi)); vTags.push_back(Form("DESY19_phi%i_diag", phi));
// v_prtcles.push_back(Form("electron_phi%i_diag", phi));} if(control or dedx)
// p_uploader = GiveMe_Uploader (intUploader, v_datafiles.back());
// if (control)		Control
...
// v_prtcles.back()); 		if (Draw_control)	DrawOut_Control
// (drawoutScanPath, vTags.back(), comment, selectionSet, 1); 		if
// (dedx) p_dEdx->Reconstruction(); 		if (Draw_Checks)
// DrawOut_Checks (drawoutScanPath, v_datafiles.back(), vTags.back(), comment);
// if (Draw_dEdx)	DrawOut_Methods (drawoutScanPath, vTags.back(), comment,
// 1, v_prtcles.back()); 		delete p_uploader;
//	}
//	}
...
// zDrift+1));
// vTags.push_back(Form("CERN23_MC_z%i00", zDrift+1));
// v_prtcles.push_back(Form("MC %i0cm", zDrift+1);
//		v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_NoOpt_iter4.root",
// zDrift+1));
// vTags.push_back(Form("CERN23_MC_z%i00", zDrift+1));
// v_prtcles.push_back(Form("MC %i0cm NoOpt", zDrift+1));
//		//
...
// selectionSet, 1); 		if (dedx)
// p_dEdx->Reconstruction(); 		if (Draw_Checks) DrawOut_Checks
...
//		if (Draw_control)	DrawOut_Control
//(drawoutScanPath, vTags.back(), comment,
// selectionSet, 8);
//	}
//	}
...
// vTags.push_back(Form("DESY21_zm40_PT%i", PT));			}
// else {v_datafiles.push_back(dataScanPath +
...
// (Draw_control) DrawOut_Control (drawoutScanPath, vTags.back(), comment,
// selectionSet, 1); 		if (dedx)
// p_dEdx->Reconstruction(); 		if (Draw_dEdx) DrawOut_Methods
// (drawoutScanPath, vTags.back(), comment, 1, v_prtcles.back());
//		}
