...
void Reconstruction::Monitoring()
{
   ...
   if (prototype) {
      ...
      for (int iFile : part_arr) {
         ...
         if (iFile == 3) {
            v_datafiles.push_back(data_scanpath +
                                  "ERAM18_350V_412ns_mu-_1GeV_iter0.root");
   ...
   if (CERN_Escan) {
      ...
      for (int iEnergy = 0; iEnergy < (int)std::size(v_valstr); iEnergy++) {
         ...
         v_datafiles.push_back(data_scanpath + "All_ERAMS_350V_412ns_" +
                               v_valstr[iEnergy] + v_suffix[iEnergy]);
         v_tags.push_back(
            Form("%s_%s_%s", testbeam.c_str(), scan.c_str(), v_valstr[iEnergy].c_str()));
   ...
   if (CERN_drift) {
      ...
      for (int iz = 0; iz < NFiles; iz++) {
         ...
         v_datafiles.push_back(data_scanpath + "All_ERAMS_350V_412ns_e+_0p5GeV_25V_z" +
                               z + "_iter4.root");
   ...
   if (DESY_drift) {
      ...
      // v_valstr.insert(v_valstr.end(), {"m40", "060", "160", "260", "360", "460", "560",
      //                                  "660", "760", "860"});
      int PT_arr[] = {412}; // 200, 412
      v_valint.insert(v_valint.end(), {50, 950});
      v_valstr.insert(v_valstr.end(), {"m40", "860"});
      for (int iPT : PT_arr) {
         PT = iPT; // PT is a shared variable in the namespace so it's important to update
                   // it here
         Settings("DESY21", "Drift", Form("Drift_PT%i", PT), Form("%i ns peak", PT),
   ...
   if (DESY_row) {
      Settings("DESY21", "", "Row", "", "Vertical beam position (mm)", 1, 0, 412, 310, 90,
               50);
      v_valint.insert(v_valint.end(),
                      {-140, -120, -100, -80, -60, -40, 0, 20, 40, 60, 80});
      v_valstr.insert(v_valstr.end(), {"m140", "m120", "m100", "m80", "m60", "m40", "0",
                                       "20", "40", "60", "80"});

      for (int y = 0; y < (int)std::size(v_valstr); y++) {
         v_datafiles.push_back(data_scanpath +
                               Form("Row_scan/Y%s_Z0_iter9.root", v_valstr[y].c_str()));
         v_tags.push_back(
            Form("%s_%s_Y%s", testbeam.c_str(), scan.c_str(), v_valstr[y].c_str()));
         DefaultAnalysis();
      }
      if (Draw_DESY21SingleScan)
         DrawSingleScan();
   ...
   if (DESY_phi) {
      ...
      std::vector<int> v_valdrift = {550};
      std::vector<std::string> v_strdrift = {"460"};
      ...
      for (int iz = 0; iz < (int)v_valdrift.size(); iz++) {
         ...
         Settings("DESY21", "Phi", Form("Phi_Z%s", z),
                  Form("%2i cm drift", v_valdrift[iz] / 10),
         ...
         for (int ifile = 0; ifile < (int)std::size(v_valint); ifile++) {
            ...
            int phireal = v_valintreal[ifile];
            if (ifile == 0) {
               v_datafiles.push_back(
                  data_scanpath +
               ...
               v_tags.push_back(
                  Form("%s_%s_Phi%i", testbeam.c_str(), scan.c_str(), phireal));
            } else {
               v_datafiles.push_back(
                  data_scanpath + Form("Phi_scan_z%s/phi_200_%i_z%s_ym60_diag_iter9.root",
                                       z, phireal, z));
               v_tags.push_back(
                  Form("%s_%s_Phi%i_diag", testbeam.c_str(), scan.c_str(), phireal));
            }
   ...
   if (DESY_theta) {
      ...
      for (int ifile = 0; ifile < (int)std::size(v_valint); ifile++) {
         ...
         v_datafiles.push_back(
            data_scanpath + Form("Theta_scan/theta_%s_02T_z460_ym60_iter9.root", theta));
...
void Reconstruction::Settings(const std::string &testbeam_name,
                              const std::string &metascan_name,
...
// (drawout_scanpath, v_tags.back(), comment, v_datafiles.back(), selectionSet,
// p_uploader, moduleCase, 0, PT, TB, v_prtcles.back()); 		if (Draw_control)
// DrawOut_Control (drawout_scanpath, v_tags.back(), comment, selectionSet, 1); 		if
// (dedx)				p_dEdx->Reconstruction(); 		if (Draw_dEdx)			DrawOut_Methods
//(drawout_scanpath, v_tags.back(), comment, 1, v_prtcles.back()); 		if (Draw_Checks)
//DrawOut_Checks (drawout_scanpath, v_datafiles.back(), v_tags.back(), comment);
//		}
//	}
//	}
...
//	if (control or dedx) p_lut = new
//LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root",
// Dt, PT)); 	int phi_val[] = {0, 10, 20, 30, 40, 45}; 	int NFiles = 6; drawout_scanpath
// = drawout_path +
// "/DESY19_phi/"; 	MakeMyDir(drawout_scanpath); 	for (int ifile = 3; ifile < NFiles;
// ifile++){ 		int phi = phi_val[ifile];
//		if(ifile < 3) {
//v_datafiles.push_back(Form("../Data_DESY19/Phi_scan/phi_412_%i_iter0.root", phi));
// v_tags.push_back(Form("DESY19_phi%i", phi));
...
// v_tags.push_back(Form("DESY19_phi%i_diag", phi));
// v_prtcles.push_back(Form("electron_phi%i_diag", phi));} if(control or dedx) p_uploader
// = GiveMe_Uploader (intUploader, v_datafiles.back()); 		if (control)		Control
//(drawout_scanpath, v_tags.back(), comment, v_datafiles.back(), selectionSet, p_uploader,
//moduleCase, 0, PT, TB,
// v_prtcles.back()); 		if (Draw_control)	DrawOut_Control			(drawout_scanpath,
// v_tags.back(), comment, selectionSet, 1); 		if (dedx)
// p_dEdx->Reconstruction(); 		if (Draw_Checks)	DrawOut_Checks (drawout_scanpath,
// v_datafiles.back(), v_tags.back(), comment); 		if (Draw_dEdx)	DrawOut_Methods
...
//		//
//v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_MD_RC100_v2_iter4.root",
//zDrift+1));
// v_tags.push_back(Form("CERN23_MC_z%i00", zDrift+1)); v_prtcles.push_back(Form("MC
// %i0cm", zDrift+1);
//		v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_NoOpt_iter4.root",
//zDrift+1));
// v_tags.push_back(Form("CERN23_MC_z%i00", zDrift+1)); v_prtcles.push_back(Form("MC %i0cm
// NoOpt", zDrift+1));
//		//
//v_datafiles.push_back(Form("../Data_MC/MC_zscan/z_400_nomDrift_%i0cm_RC100_iter4.root",
//zDrift+1));
// v_tags.push_back(Form("CERN23_MC_z%i00_old", zDrift+1)); v_prtcles.push_back(Form("MC
// %i0cm old", zDrift+1);
//		if(control or dedx) p_uploader = GiveMe_Uploader (intUploader, v_datafiles.back());
//		if (control)		Control		(drawout_scanpath, v_tags.back(), comment, v_datafiles.back(),
//selectionSet,
// p_uploader, moduleCase, 0, PT, TB, v_prtcles.back()); 		if (Draw_control)
// DrawOut_Control (drawout_scanpath, v_tags.back(), comment, selectionSet, 1); 		if
// (dedx)			p_dEdx->Reconstruction(); 		if (Draw_Checks) DrawOut_Checks
// (drawout_scanpath, v_datafiles.back(), v_tags.back(), comment); 		if (Draw_dEdx)
// DrawOut_Methods
...
//		if (Draw_control)	DrawOut_Control			(drawout_scanpath, v_tags.back(), comment,
//selectionSet, 8);
//	}
//	}
...
// v_tags.push_back(Form("DESY21_zm40_PT%i", PT));			} 		else
// {v_datafiles.push_back(data_scanpath +
// Form("zscan_PT%i_139V/z_360_139_%i_02T_26_%i60_iter0.root", PT, PT, zDrift));
// v_tags.push_back(Form("DESY21_z%i60_PT%i", zDrift, PT));
...
// (drawout_scanpath, v_tags.back(), comment, 1, v_prtcles.back());
//		}
//		if(Draw_Resolution)	DrawOut_Zscan	(drawout_path +
