...
void Reconstruction::Monitoring()
{
  ...
  if(prototype){
	...
	if(CERN_Escan){
		Settings("T2_CERN22_Event", "../OUT_Reconstruction/CERN22_Energy_Scan/", 2, -1, 412, 350, 415, 40);
		if (control or dedx) p_lut = new LUT(Form("~/Documents/Code/Python/LUT/LUT_Dt%i_PT%i_nphi150_nd150_nRC41_nZ21.root", Dt, PT));
		...
		for (int iFile : part_arr) {
			...
			if(dedx) p_uploader = 		GiveMe_Uploader (intUploader, v_dataFiles.back());
  ...
  if(DESY_phi){
    ...
    for (int iz = 0; iz < (int)std::size(z_arr); iz++){
      ...
      for (int ifile = 5; ifile < 8; ifile++){
        ...
        if(control or dedx) p_uploader = GiveMe_Uploader (intUploader, v_dataFiles.back());
        if (control)      Control                   (outDir, v_tags.back(), comment_phi, v_dataFiles.back(), selectionSet, p_uploader, moduleCase, 0, PT, TB, v_prtcles.back());
        if (DO_control)   DrawOut_Control           (outDir, v_tags.back(), comment_phi, selectionSet, 1);
        if (dedx)          p_dEdx->Reconstruction();
        if (DO_Checks)    DrawOut_Checks            (outDir, v_dataFiles.back(), v_tags.back(), comment);
        if (DO_dEdx)   DrawOut_Methods           (outDir, v_tags.back(), comment_phi, 1, v_prtcles.back());
        delete p_uploader;
