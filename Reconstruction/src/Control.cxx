 ...
 * Project: dEdxRecon
 *
 * Brief: Implementation of the top-level Control(...) function that sets up
 *        and dispatches reconstruction, plotting and monitoring tasks based
 *        on provided configuration and command-line parameters.
 *
 * Contents: Control orchestration logic.
 *
 * Notes: Control ties together multiple modules; changes here affect run
 *        orchestration.
 ***************************************************************************/
...
void Control(const std::string &OutDir, std::string const &Tag,
  ...
  for (int iMod = 0; iMod < 8; iMod++) {
    TH1F *h1f_TL_Raw =
        new TH1F(Form("h1f_TL_Raw_%i", iMod),
                 Form("T_{Leading} Raw (Mod %i);Time bin;Count", iMod), 511,
                 -0.5, 510.5);
    TH1F *h1f_TL_Sel =
        new TH1F(Form("h1f_TL_Sel_%i", iMod),
                 Form("T_{Leading} Cut (Mod %i);Time bin;Count", iMod), 511,
                 -0.5, 510.5);
    TH1F *h1f_PM_Raw = new TH1F(
        Form("h1f_PM_Raw_%i", iMod),
        Form("Pad Multiplicity Raw (Mod %i);Number of pads;Count", iMod), 11,
        -0.5, 10.5);
    TH1F *h1f_PM_Sel = new TH1F(
        Form("h1f_PM_Sel_%i", iMod),
        Form("Pad Multiplicity Cut (Mod %i);Number of pads;Count", iMod), 11,
        -0.5, 10.5);
    TH1F *h1f_APM_Raw =
        new TH1F(Form("h1f_APM_Raw_%i", iMod),
                 Form("Average Pad Multiplicity Raw (Mod %i);average "
                      "pad integral (ADC counts);Count",
                      iMod),
                 44, -0.5, 10.5);
    TH1F *h1f_APM_Sel =
        new TH1F(Form("h1f_APM_Sel_%i", iMod),
                 Form("Average Pad Multiplicity Cut (Mod %i);average "
                      "pad integral (ADC counts);Count",
                      iMod),
                 44, -0.5, 10.5);
    TH1F *h1f_API_Raw =
        new TH1F(Form("h1f_API_Raw_%i", iMod),
                 Form("Average of the pad integral Raw (Mod %i);Mean "
                      "pad integral (ADC counts);Count",
                      iMod),
                 400, -20, 255);
    TH1F *h1f_API_Sel =
        new TH1F(Form("h1f_API_Sel_%i", iMod),
                 Form("Average of the pad integral Cut (Mod %i);Mean "
                      "pad integral (ADC counts);Count",
                      iMod),
                 400, -20, 255);
    TH1F *h1f_Qpad_Raw = new TH1F(
        Form("h1f_Qpad_Raw_%i", iMod),
        Form("Q_{pad} Raw (Mod %i);Amplitude (ADC counts);Count", iMod), 400,
        -100, 3900);
    TH1F *h1f_Qpad_Sel = new TH1F(
        Form("h1f_Qpad_Sel_%i", iMod),
        Form("Q_{pad} Cut (Mod %i);Amplitude (ADC counts);Count", iMod), 400,
        -100, 3900);
    TH1F *h1f_Qlead_Raw = new TH1F(
        Form("h1f_Qlead_Raw_%i", iMod),
        Form("Q_{lead} Raw (Mod %i);Amplitude (ADC counts);Count", iMod), 200,
        -100, 3900);
    TH1F *h1f_Qlead_Sel = new TH1F(
        Form("h1f_Qlead_Sel_%i", iMod),
        Form("Q_{lead} Cut (Mod %i);Amplitude (ADC counts);Count", iMod), 200,
        -100, 3900);
    TH1F *h1f_Qneigh_Raw = new TH1F(
        Form("h1f_Qneigh_Raw_%i", iMod),
        Form("Q_{neighbours} Raw (Mod %i);Amplitude (ADC counts);Count", iMod),
        200, -100, 3900);
    TH1F *h1f_Qneigh_Sel = new TH1F(
        Form("h1f_Qneigh_Sel_%i", iMod),
        Form("Q_{neighbours} Cut (Mod %i);Amplitude (ADC counts);Count", iMod),
        200, -100, 3900);
    TH1F *h1f_Qclus_Raw = new TH1F(
        Form("h1f_Qclus_Raw_%i", iMod),
        Form("Q_{cluster} Raw (Mod %i);Amplitude (ADC counts);Count", iMod),
        120, -100, 3900);
    TH1F *h1f_Qclus_Sel = new TH1F(
        Form("h1f_Qclus_Sel_%i", iMod),
        Form("Q_{cluster} Cut (Mod %i);Amplitude (ADC counts);Count", iMod),
        120, -100, 3900);
    TH1F *h1f_CM_Raw = new TH1F(Form("h1f_CM_Raw_%i", iMod),
                                Form("Number of clusters per module Raw (Mod "
                                     "%i);Number of clusters;Count",
                                     iMod),
                                37, -0.5, 36.5);
    TH1F *h1f_CM_Sel = new TH1F(Form("h1f_CM_Sel_%i", iMod),
                                Form("Number of clusters per module Cut (Mod "
                                     "%i);Number of clusters;Count",
                                     iMod),
                                37, -0.5, 36.5);
    ...
    TH2F *h2f_LeadPos_Raw =
        new TH2F(Form("h2f_LeadPos_Raw_%i", iMod),
                 Form("Position of leading pads in ERAM (Mod %i);iX;iY", iMod),
                 36, -0.5, 35.5, 32, -0.5, 31.5);
    ...
    TH2F *h2f_Theta_Sel =
        new TH2F(Form("h2f_Theta_Sel_%i", iMod),
                 ...
                 36, -0.5, 35.5, 80, 0, 1010);
    TH2F *h2f_rho_DT_Raw =
        new TH2F(Form("h2f_rho_DT_Raw_%i", iMod),
                 Form("A_{1st neighbour} / A_{leading} vs T_{1st neighbour} - "
                      "T_{leading} (Mod "
                      "%i);T_{1st neighbour} - T_{leading};A_{1st neighbour} / "
                      "A_{leading}",
                      iMod),
                 101, -20, 80, 200, 0, 1);
    TH2F *h2f_rho_DT_Sel =
        new TH2F(Form("h2f_rho_DT_Sel_%i", iMod),
                 Form("A_{1st neighbour} / A_{leading} vs T_{1st neighbour} - "
                      "T_{leading} (Mod "
                      "%i);T_{1st neighbour} - T_{leading};A_{1st neighbour} / "
                      "A_{leading}",
  ...
  Init_selection(SelectionSet, aSelector, Tag, pUploader, NbrOfMod,
                 Data_to_Use);
  ...
  for (int iEvent = 0; iEvent < NEvent; iEvent++) {
    ...
    for (int iMod = 0; iMod < nMod; iMod++) {
      ...
      for (int iC = 0; iC < NClusters; iC++) {
        ...
        TH1F *h1f_clus =
            new TH1F("h1f_WF_cluster", "h1f_WF_cluster", 510, 0, 510);
        ...
        if (PT == 412 and TB == 50) {
          T0 = 45;
          z_calc = 3.90 * (TL - T0);
        } // 45	= 37(time shift) +	8(PT) from David
        ...
        for (int iP = 0; iP < NPads; iP++) {
          ...
          if (iP == NPads - 1)
            v_h1f_Qlead_Raw[ModuleNber]->Fill(AP);
          else
            v_h1f_Qneigh_Raw[ModuleNber]->Fill(AP);
    ...
    for (int iMod = 0; iMod < nMod; iMod++) {
      ...
      for (int iC = 0; iC < NClusters; iC++) {
        ...
        for (int iP = 0; iP < NPads; iP++) {
          ...
          float pad_integral = h1f_WF->Integral();
          if (iP == NPads - 1)
            v_h1f_Qlead_Sel[ModuleNber]->Fill(AP);
          else
            v_h1f_Qneigh_Sel[ModuleNber]->Fill(AP);
  ...
  if (EventFile.find("All_ERAMS") != std::string::npos) {
    std::cout << "Raw		events with at least 4 modules: "
              << nEvt_raw4mod << std::endl;
  ...
  TFile *pfileROOT = new TFile(
      TString(OUTDirName + "1_" + Tag + "_Control" + Comment + ".root"),
      "RECREATE");
  for (int iMod = 0; iMod < nMod; iMod++) {
  ...
  delete pfileROOT;
  for (int iMod = 0; iMod < 8; iMod++) {
    delete v_h1f_PM_Raw[iMod];
    v_h1f_PM_Raw[iMod] = 0;
    delete v_h1f_PM_Sel[iMod];
    v_h1f_PM_Sel[iMod] = 0;
    delete v_h1f_TL_Raw[iMod];
    v_h1f_TL_Raw[iMod] = 0;
    delete v_h1f_TL_Sel[iMod];
    v_h1f_TL_Sel[iMod] = 0;
    delete v_h1f_APM_Raw[iMod];
    v_h1f_APM_Raw[iMod] = 0;
    delete v_h1f_APM_Sel[iMod];
    v_h1f_APM_Sel[iMod] = 0;
    delete v_h1f_API_Raw[iMod];
    v_h1f_API_Raw[iMod] = 0;
    delete v_h1f_API_Sel[iMod];
    v_h1f_API_Sel[iMod] = 0;
    delete v_h1f_Qpad_Raw[iMod];
    v_h1f_Qpad_Raw[iMod] = 0;
    delete v_h1f_Qpad_Sel[iMod];
    v_h1f_Qpad_Sel[iMod] = 0;
    delete v_h1f_Qlead_Raw[iMod];
    v_h1f_Qlead_Raw[iMod] = 0;
    delete v_h1f_Qlead_Sel[iMod];
    v_h1f_Qlead_Sel[iMod] = 0;
    delete v_h1f_Qneigh_Raw[iMod];
    v_h1f_Qneigh_Raw[iMod] = 0;
    delete v_h1f_Qneigh_Sel[iMod];
    v_h1f_Qneigh_Sel[iMod] = 0;
    delete v_h1f_Qclus_Raw[iMod];
    v_h1f_Qclus_Raw[iMod] = 0;
    delete v_h1f_Qclus_Sel[iMod];
    v_h1f_Qclus_Sel[iMod] = 0;
    delete v_h1f_CM_Raw[iMod];
    v_h1f_CM_Raw[iMod] = 0;
    delete v_h1f_CM_Sel[iMod];
    v_h1f_CM_Sel[iMod] = 0;
    delete v_h1f_Eff_Raw[iMod];
    v_h1f_Eff_Raw[iMod] = 0;
    delete v_h1f_Eff_Sel[iMod];
    v_h1f_Eff_Sel[iMod] = 0;
    delete v_h2f_LeadPos_Raw[iMod];
    v_h2f_LeadPos_Raw[iMod] = 0;
    delete v_h2f_LeadPos_Sel[iMod];
    v_h2f_LeadPos_Sel[iMod] = 0;
    delete v_h2f_Theta_Raw[iMod];
    v_h2f_Theta_Raw[iMod] = 0;
    delete v_h2f_Theta_Sel[iMod];
    v_h2f_Theta_Sel[iMod] = 0;
    delete v_h2f_rho_DT_Raw[iMod];
    v_h2f_rho_DT_Raw[iMod] = 0;
    delete v_h2f_rho_DT_Sel[iMod];
    v_h2f_rho_DT_Sel[iMod] = 0;
  }
  v_h1f_TL_Raw.clear();
  v_h1f_TL_Sel.clear();
  v_h1f_PM_Raw.clear();
