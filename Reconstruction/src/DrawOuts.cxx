...
Reconstruction::DrawOuts::DrawOuts()
    : v_fFiles(), v_fTrees(), v_fEvents(), v_fnentries(), p_recoevent(nullptr),
      p_recomodule(nullptr), p_recocluster(nullptr), p_recopad(nullptr),
      fpCanvas(nullptr), drawMultiScans(0) {
  SetStyle();
}
...
// Main constructor
Reconstruction::DrawOuts::DrawOuts(
    const std::vector<std::string> &v_inputFiles) {
...
void Reconstruction::DrawOuts::DESY21ScanFill() {
  ...
  // If only one scan must be drawn, but belongs to a multi-scan, set the index
  // to the first scan index that corresponds to this scan
  int iStartScan = 0;
  if (drawMultiScans == 0)
  ...
  for (int iscan = 0; iscan < nScans; iscan++) {
    ...
    for (int irun = 0; irun < nRuns; irun++) {
      ...
      if (drawMultiScans == 0)
        index = irun;
      // For multi-scans, shift the index to take into account the number of
      // scans iterated before this one
      else
        index = iscan * nRuns + irun;

      v_fptf1_WF.push_back((TF1 *)v_fFiles[iscan * nRuns + irun]
                               ->Get<TH1F>("ph1f_WF")
                               ->GetFunction("gausn"));
      v_fptf1_XP.push_back((TF1 *)v_fFiles[iscan * nRuns + irun]
                               ->Get<TH1F>("ph1f_XP")
                               ->GetFunction("gausn"));
...
void Reconstruction::DrawOuts::DESY21ScanDraw() {
  ...
  // Drawing settings
  if (drawMultiScans == 1)
    foutputFile = drawoutMultiScanPath + testbeam + "_" + multiScanName +
                  comment + methodsString + "_Dt" + std::to_string(Dt) + ".pdf";
  else
  ...
  TLegend legMulti(0.94 - 0.21 * fnMethods, 0.84 - 0.04 * nLegEntries, 0.94,
                   0.92);
  if (fnMethods == 2)
  ...
  for (int i = 0; i < nScans; i++) {
    if (fwhichMethods == 0 or fwhichMethods == 1) // Only WF or both methods
      legMulti.AddEntry(v_fpTGE_reso_WF[i], Form("%s", v_scanspec[i].c_str()),
                        "p");
    if (fwhichMethods == 0 or fwhichMethods == 2) // Only XP or both methods
      legMulti.AddEntry(v_fpTGE_reso_XP[i], Form("%s", v_scanspec[i].c_str()),
                        "p");
  }
  ...
  tge->SetNameTitle("fpTGE_reso_WF",
  ...
  for (int i = firstScan; i < (int)v_fpTGE_reso_WF.size(); i++) {
    if (drawMultiScans) {
      ...
      Graphic_setup(v_fpTGE_reso_WF[i], marksize, markers[4], colors[2],
                    linesize, colors[2]);
      Graphic_setup(v_fpTGE_reso_XP[i], marksize, markers[3], colors[3],
                    linesize, colors[3]);
    }
  ...
  if (drawMultiScans == 0 and
      fwhichMethods == 0) // Single scan and both methods
    legSingle.Draw();
  if (drawMultiScans)
    legMulti.Draw();
  ...
  for (int i = firstScan; i < (int)v_fpTGE_mean_WF.size(); i++) {
    if (drawMultiScans) {
      ...
      Graphic_setup(v_fpTGE_mean_WF[i], marksize, markers[4], colors[2],
                    linesize, colors[2]);
      Graphic_setup(v_fpTGE_mean_XP[i], marksize, markers[3], colors[3],
                    linesize, colors[3]);
    }
    if (fwhichMethods == 1) { // Only draw WF
      if (i == firstScan)
  ...
  for (int i = firstScan; i < (int)v_fpTGE_std_WF.size(); i++) {
    if (drawMultiScans) {
      Graphic_setup(v_fpTGE_std_WF[i], marksize, markers[2 * i], colors[2 * i],
                    linesize, colors[2 * i]);
      Graphic_setup(v_fpTGE_std_XP[i], marksize, markers[2 * i + 1],
                    colors[2 * i + 1], linesize, colors[2 * i + 1]);
    } else {
      Graphic_setup(v_fpTGE_std_WF[i], marksize, markers[4], colors[2],
                    linesize, colors[2]);
      Graphic_setup(v_fpTGE_std_XP[i], marksize, markers[3], colors[3],
                    linesize, colors[3]);
    }
    if (fwhichMethods == 1) { // Only draw WF
      if (i == firstScan)
...
void Reconstruction::DrawOuts::CERN22ScanFill() {
  ...
  for (int iscan = 0; iscan < 4; iscan++) { // 4 types of particles
    ...
    v_fpTGE_reso_WF.push_back(new TGraphErrors());
    v_fpTGE_reso_XP.push_back(new TGraphErrors());
    v_fptf1_BB.push_back(
        BetheBlochExp(0.25, 20, v_mass[iscan], particles[iscan]));

    for (int irun = 0; irun < v_runs[iscan]; irun++) {

      fptf1_WF = (TF1 *)v_fFiles[itotalrun]->Get<TH1F>("ph1f_WF")->GetFunction(
          "gausn");
      fptf1_XP = (TF1 *)v_fFiles[itotalrun]->Get<TH1F>("ph1f_XP")->GetFunction(
          "gausn");
      ...
      v_fpTGE_mean_WF[iscan]->SetPoint(irun, vScanVals[itotalrun],
                                       mean_WF * keV);
      v_fpTGE_mean_XP[iscan]->SetPoint(irun, vScanVals[itotalrun],
                                       mean_XP * keV);
...
void Reconstruction::DrawOuts::CERN22ScanDraw() {
  ...
  // Drawing settings
  foutputFileWF = drawoutScanPath + scanName + comment + "_WF_Dt" +
                  std::to_string(Dt) + ".pdf";
  foutputFileXP = drawoutScanPath + scanName + comment + "_XP_Dt" +
                  std::to_string(Dt) + ".pdf";
  ...
  v_fpTGE_reso_WF[0]->SetNameTitle(
      "fpTGE_reso_WF", Form(";%s;Resolution [%%]", runvarstr.c_str()));
  for (int i = 0; i < 4; i++) {
  ...
  v_fpTGE_reso_XP[0]->SetNameTitle(
      "fpTGE_reso_XP", Form(";%s;Resolution [%%]", runvarstr.c_str()));
  for (int i = 0; i < 4; i++) {
...
void Reconstruction::DrawOuts::Control() {
  ...
  // Prepare histograms
  TH1I *ph1i_nmodules = new TH1I(
      "ph1i_nmodules", "Number of modules;N_{modules};Counts", 8, 0, 8);
  TH1I *ph1i_nmodulesSel =
      new TH1I("ph1i_nmodulesSel",
               "Number of modules (after cut);N_{modules};Counts", 8, 0, 8);
  TH1I *ph1i_nclusters = new TH1I(
      "ph1i_nclusters", "Number of clusters;N_{clusters};Counts", 55, 0, 55);
  TH1I *ph1i_nclustersSel =
      new TH1I("ph1i_nclustersSel",
               "Number of clusters (after cut);N_{clusters};Counts", 55, 0, 55);
  TH1I *ph1i_npads =
      new TH1I("ph1i_npads", "Number of pads;N_{pads};Counts", 10, 0, 10);
  TH1I *ph1i_npadsSel = new TH1I(
      "ph1i_npadsSel", "Number of pads (after cut);N_{pads};Counts", 10, 0, 10);

  TH2I *ph2i_heatmap =
      new TH2I("ph2i_heatmap", "Events heatmap;iX;iY;", 144, 0, 143, 64, 0, 63);
  TH1I *ph1i_TLead = new TH1I(
      "ph1i_TLead", "T_{max} of leading pad;T_{max};Counts", 511, 0, 510);
  TH1I *ph1i_TLeadSel = new TH1I(
      "ph1i_TLeadSel", "T_{max} of leading pad (after cut);T_{max};Counts", 511,
      0, 510);
  TH1I *ph1i_PadMult =
      new TH1I("ph1i_PadMult", "Pad multiplicity;N_{pads};Counts", 11, 0, 10);
  TH1I *ph1i_PadMultSel =
      new TH1I("ph1i_PadMultSel",
               "Pad multiplicity (after cut);N_{pads};Counts", 11, 0, 10);
  TH1F *ph1f_AvgPadMult =
      new TH1F("ph1f_AvgPadMult", "Average pad multiplicity;N_{pads};Counts",
               100, 0, 10);
  TH1F *ph1f_AvgPadMultSel = new TH1F(
      "ph1f_AvgPadMultSel",
      "Average pad multiplicity (after cut);N_{pads};Counts", 100, 0, 10);
  TH1F *ph1f_ADCmax =
      new TH1F("ph1f_ADCmax", "ADC max;ADC counts/cm;Counts", 400, 0, 4000);
  TH1F *ph1f_ADCmaxSel =
      new TH1F("ph1f_ADCmaxSel", "ADC max (after cut);ADC counts/cm;Counts",
               400, 0, 4000);
  TH1F *ph1f_ALead = new TH1F(
      "ph1f_ALead", "Leading pad amplitude;ADC counts/cm;Counts", 400, 0, 4000);
  TH1F *ph1f_ALeadSel = new TH1F(
      "ph1f_ALeadSel", "Leading pad amplitude (after cut);ADC counts/cm;Counts",
      400, 0, 4000);
  TH1F *ph1f_ANeighbour =
      new TH1F("ph1f_ANeighbour",
               "Neighbouring pad amplitude;ADC counts/cm;Counts", 400, 0, 4000);
  TH1F *ph1f_ANeighbourSel =
      new TH1F("ph1f_ANeighbourSel",
               "Neighbouring pad amplitude (after cut);ADC counts/cm;Counts",
               400, 0, 4000);
  TH1F *ph1f_yCluster = new TH1F(
      "ph1f_yCluster",
      "Vertical cluster position;vertical position (cm);Normalized counts", 100,
      4, 14);
  ...
  TH1F *ph1f_dyClus = new TH1F(
      "ph1f_dyClus", "\\Deltay position};\\Deltay (cm);Normalized counts", 100,
      -5, 5);
  TH1F *ph1f_dyWeight =
      new TH1F("ph1f_dyWeight",
               "\\Deltay position;\\Deltay (cm);Normalized counts", 100, -5, 5);
  ...
  for (int i = 0; i < 8; i++) {
    v_TLead.push_back(
        new TH1I(Form("ph1i_TLead_%i", i),
                 Form("T_{max} of leading pad (module %i);T_{max};Counts", i),
                 511, 0, 510));
    v_TLeadSel.push_back(new TH1I(
        Form("ph1i_TLeadSel_%i", i),
        Form("T_{max} of leading pad (module %i) (after cut);T_{max};Counts",
             i),
        511, 0, 510));
    v_PadMult.push_back(new TH1I(
        Form("ph1i_PadMult_%i", i),
        Form("Pad multiplicity (module %i);N_{pads};Counts", i), 100, 0, 100));
    v_PadMultSel.push_back(new TH1I(
        Form("ph1i_PadMultSel_%i", i),
        Form("Pad multiplicity (module %i) (after cut);N_{pads};Counts", i),
        100, 0, 100));
    v_AvgPadMult.push_back(new TH1F(
        Form("ph1f_AvgPadMult_%i", i),
        Form("Average pad multiplicity (module %i);N_{pads};Counts", i), 100, 0,
        100));
    v_AvgPadMultSel.push_back(new TH1F(
        Form("ph1f_AvgPadMultSel_%i", i),
        Form("Average pad multiplicity (module %i) (after cut);N_{pads};Counts",
             i),
        100, 0, 100));
    v_ADCmax.push_back(new TH1F(
        Form("ph1f_ADCmax_%i", i),
        Form("ADC max (module %i);ADC counts/cm;Counts", i), 400, 0, 4000));
    v_ADCmaxSel.push_back(new TH1F(
        Form("ph1f_ADCmaxSel_%i", i),
        Form("ADC max (module %i) (after cut);ADC counts/cm;Counts", i), 400, 0,
        4000));
    v_ALead.push_back(new TH1F(
        Form("ph1f_ALead_%i", i),
        Form("Leading pad amplitude (module %i);ADC counts/cm;Counts", i), 400,
        0, 4000));
    v_ALeadSel.push_back(new TH1F(Form("ph1f_ALeadSel_%i", i),
                                  Form("Leading pad amplitude (module %i) "
                                       "(after cut);ADC counts/cm;Counts",
                                       i),
                                  400, 0, 4000));
    v_ANeighbour.push_back(new TH1F(
        Form("ph1f_ANeighbour_%i", i),
    ...
    v_ANeighbourSel.push_back(
        new TH1F(Form("ph1f_ANeighbourSel_%i", i),
                 Form("Neighbouring pad amplitude (module %i) (after cut);ADC "
                      "counts/cm;Counts",
                      i),
                 400, 0, 4000));
  }
  ...
  for (int i = 0; i < 36; i++) {
    v_yCluster.push_back(
        new TH1F(Form("ph1f_yCluster_%i", i),
                 Form("Vertical cluster position (column %i);vertical "
                      "position (cm);Normalized counts",
                      i),
                 100, 4, 14));
    v_yWeight.push_back(
        new TH1F(Form("ph1f_yWeight_%i", i),
                 Form("Vertical cluster position (column %i);vertical "
                      "position (cm);Normalized counts",
                      i),
                 100, 4, 14));
    v_dyClus.push_back(new TH1F(
        Form("ph1f_dyClus_%i", i),
        Form("#Deltay (column %i);#Deltay (cm);Counts", i), 100, -5, 5));
    v_dyWeight.push_back(new TH1F(
        Form("ph1f_dyWeight_%i", i),
        Form("#Deltay (column %i);#Deltay (cm);Counts", i), 100, -5, 5));
  }
  ...
  for (int i = 1; i < 35; ++i) {
    ...
    Graphic_setup(v_yWeight[i], 0.5, 1, kOrange - 3, 3, kOrange - 3,
                  kOrange - 3, 0.2);
    v_yWeight[i]->Draw("HIST");
    Graphic_setup(v_yCluster[i], 0.5, 1, kBlue + 2, 3, kBlue + 2, kBlue + 2,
                  0.2);
  ...
  fpCanvas->Clear();
  float maxY =
      1.1 * std::max(ph1f_yCluster->GetMaximum(), ph1f_yWeight->GetMaximum());
  Graphic_setup(ph1f_yWeight, 0.5, 1, kOrange - 3, 3, kOrange - 3, kOrange - 3,
                0.2);
  ...
  for (int i = 1; i < 35; ++i) {
    float maxY =
        1.1 * std::max(v_dyClus[i]->GetMaximum(), v_dyWeight[i]->GetMaximum());
    Graphic_setup(v_dyWeight[i], 0.5, 1, kOrange - 3, 3, kOrange - 3,
                  kOrange - 3, 0.2);
  ...
  Graphic_setup(ph1f_dyWeight, 0.5, 1, kOrange - 3, 3, kOrange - 3, kOrange - 3,
                0.2);
...
void Reconstruction::DrawOuts::EnergyLoss(const int &methods) {
  ...
  // Prepare histograms
  TH1F *ph1f_GP1 =
      new TH1F("ph1f_GP1", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_GP2 =
      new TH1F("ph1f_GP2", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_GP3 =
      new TH1F("ph1f_GP3", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_GP4 =
      new TH1F("ph1f_GP4", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_GP5 =
      new TH1F("ph1f_GP5", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_rhoC =
      new TH1F("ph1f_rhoC", ";LUT ratio for crossed pads;Counts", 100, 0, 2);
  TH1F *ph1f_rhoL =
      new TH1F("ph1f_rhoL", ";LUT ratio for leading pad;Counts", 100, 0, 2);
  TH1F *ph1f_TmaxCrossed = new TH1F(
      "ph1f_TmaxCrossed", ";T_{max} of crossed pads;Counts", 80, 20, 100);
  TH1F *ph1f_TmaxLeading = new TH1F(
      "ph1f_TmaxLeading", ";T_{max} of leading pad;Counts", 80, 20, 100);
  TH2F *ph2f_dedxVsdxWF =
      new TH2F("ph2f_dedxVsdxWF", ";dx [mm];dE/dx [ADC counts/cm]", 100, 0, 16,
               100, 0, xmax);

  TH1F *ph1f_XP =
      new TH1F("ph1f_XP", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_WF =
      new TH1F("ph1f_WF", ";dE/dx [ADC counts/cm];Counts", 100, 0, xmax);
  TH1F *ph1f_XPnoTrunc =
  ...
  TH2F *ph2f_XPWF = new TH2F("ph2f_XPWF", ";dE/dx (WF);dE/dx (XP)", 100, 0,
                             xmax, 100, 0, xmax);
  ...
  for (int i = 0; i < 8; i++) {
    v_h1f_XP_mod.push_back(new TH1F(
        Form("ph1f_XP_%i", i),
        Form("Module %i;dE/dx [ADC counts/cm];Counts", i), 100, 0, xmax));
  ...
  for (int i = 0; i < v_fnentries.back(); i++) {
    ...
    for (int iMod = 0; iMod < NMod; iMod++) {
      ...
      for (int iC = 0; iC < NClusters; iC++) {
        ...
        ph2f_dedxVsdxWF->Fill(p_recocluster->length * 1e3,
                              p_recocluster->ADCmax_base);
        NPads = p_recocluster->NPads;
        for (int iP = 0; iP < NPads; iP++) {
          p_recopad = p_recocluster->v_pads[iP];
    ...
    if (i < 100) {
      TH1F *ph1f_GWF = new TH1F(
          Form("ph1f_GWF_%i", i),
          Form(";time bin [%d ns];ADC counts", p_recoevent->timeBinSize), 510,
          0, 510);
      ...
      for (int j = 1; j <= ph1F_GWFclone->GetNbinsX(); j++) {
        ...
        ph1f_GWFtruncatedGP1->SetBinContent(
            j, ph1F_GWFtruncGP1clone->GetBinContent(j));
      }
  ...
  TH2F *pth2fGP3param_tmp =
      dynamic_cast<TH2F *>(v_fFiles.back()->Get("pth2f_paramGP3"));
  TH2F *pth2fGP3param =
  ...
  for (int j = 1; j <= pth2fGP3param_tmp->GetNbinsX(); j++) {
    for (int k = 1; k <= pth2fGP3param_tmp->GetNbinsY(); k++) {
      pth2fGP3param->SetBinContent(j, k,
                                   pth2fGP3param_tmp->GetBinContent(j, k));
    }
  }
  TF1 *ptf1GP3param =
      dynamic_cast<TF1 *>(v_fFiles.back()->Get("ptf1_paramGP3"));
  ...
  for (int i = 0; i < 8; i++) {
    ...
    Graphic_setup(v_h1f_XP_mod[i], 0.5, 1, kMagenta + 2, 1, kMagenta - 2,
                  kMagenta, 0.2);
    Graphic_setup(v_h1f_WF_modnoTrunc[i], 0.5, 1, kBlue + 1, 1, kBlue - 2,
                  kBlue, 0.2);
    Graphic_setup(v_h1f_XP_modnoTrunc[i], 0.5, 1, kRed + 2, 1, kRed - 2, kRed,
                  0.2);
  ...
  PrintResolution(ph1f_WF, fpCanvas, xDefaultNDC - invX, 0.3, kCyan + 2, "WF");
  PrintResolution(ph1f_GP1, fpCanvas, xDefaultNDC - invX, 0.65, kOrange + 2,
                  "GP1");
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  PrintResolution(ph1f_WF, fpCanvas, xDefaultNDC - invX, 0.3, kCyan + 2, "WF");
  PrintResolution(ph1f_GP2, fpCanvas, xDefaultNDC - invX, 0.65, kOrange + 2,
                  "GP2");
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  PrintResolution(ph1f_WF, fpCanvas, xDefaultNDC - invX, 0.3, kCyan + 2, "WF");
  PrintResolution(ph1f_GP3, fpCanvas, xDefaultNDC - invX, 0.65, kOrange + 2,
                  "GP3");
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  PrintResolution(ph1f_WF, fpCanvas, xDefaultNDC - invX, 0.3, kCyan + 2, "WF");
  PrintResolution(ph1f_GP4, fpCanvas, xDefaultNDC - invX, 0.65, kOrange + 2,
                  "GP4");
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  PrintResolution(ph1f_WF, fpCanvas, xDefaultNDC - invX, 0.3, kCyan + 2, "WF");
  PrintResolution(ph1f_GP5, fpCanvas, xDefaultNDC - invX, 0.65, kOrange + 2,
                  "GP5");
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  ph1f_XP->Draw("HIST");
  ph1f_GP5->Draw("HIST sames");
  PrintResolution(ph1f_XP, fpCanvas, xDefaultNDC - invX, 0.3, kMagenta + 2,
                  "XP");
  PrintResolution(ph1f_GP5, fpCanvas, xDefaultNDC - invX, 0.65, kOrange + 2,
                  "GP5");
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (v_h1f_WF_mod[i]->GetEntries() < 100)
      continue;
    PrintResolution(v_h1f_WF_mod[i], fpCanvas, xDefaultNDC - invX, 0.3,
                    kCyan + 2, "WF");
    PrintResolution(v_h1f_XP_mod[i], fpCanvas, xDefaultNDC - invX, 0.65,
                    kMagenta + 2, "XP");
  }
  fpCanvas->SaveAs(drawout_file.c_str());
  ...
  if (drawGigaWF) {
    ...
    std::string drawoutFileGWF =
        drawout_file.substr(0, drawout_file.size() - 4);
    fpCanvas->Clear();
    gPad->SetGrid(1, 1);
    gPad->SetTopMargin(0.07);
    gPad->SetRightMargin(0.04);
    ...
    for (int i = 0; i < (int)v_ph1f_GWF.size(); i++) {
      ...
      Graphic_setup(v_ph1f_GWFtruncGP1[i], 1, 1, kBlue + 2, 2, kBlue + 2, kBlue,
                    0.2);
      // ETF
      int maxBin = v_ph1f_GWF[i]->GetMaximumBin();
      int maxGWF = v_ph1f_GWF[i]->GetMaximum();
      TH1F *pTH1F_ETF = ETF("pTH1F_ETF_" + std::to_string(i), 0, 510,
                            maxBin - PT / TB, 510, 999, PT, TB);
      ...
      // Set Y range
      float ymin = std::min(v_ph1f_GWF[i]->GetMinimum(),
                            v_ph1f_GWFtruncGP1[i]->GetMinimum());
      float ymax = std::max(v_ph1f_GWF[i]->GetMaximum(),
                            v_ph1f_GWFtruncGP1[i]->GetMaximum());
...
void Reconstruction::DrawOuts::FileComparison() {
  ...
  for (int i = 0; i < (int)v_comments.size(); i++) {
    v_h1f_XP.push_back(
        new TH1F(Form("ph1f_XP_%i", i),
                 Form("Energy loss (%s);dE/dx [ADC counts/cm];Counts",
                      v_comments[i].c_str()),
                 100, 0, 1300));
    v_h1f_WF.push_back(
        new TH1F(Form("ph1f_WF_%i", i),
                 Form("Energy loss (%s);dE/dx [ADC counts/cm];Counts",
                      v_comments[i].c_str()),
                 100, 0, 1300));
  ...
  for (int ifile = 0; ifile < (int)v_comments.size(); ifile++) {
    for (int i = 0; i < v_fnentries[ifile]; i++) {
      ...
      for (int iMod = 0; iMod < NMod; iMod++) {
        ...
        v_h1f_WF_mod[i][position]->Fill(
            v_fEvents[ifile]->v_modules[iMod]->dEdxWF);
        v_h1f_XP_mod[i][position]->Fill(
            v_fEvents[ifile]->v_modules[iMod]->dEdxXP);
      }
    }
  }
...
void Reconstruction::DrawOuts::AmplitudeVSLength() {
  ...
  ptf1CorrVlada->SetTitle(
      ";Track length in cluster [mm];Amplitude [ADC counts/cm]");
  ...
  TF1 *ptf1CorrFuncRef =
      TFile::Open(vcorrFuncPaths[1].c_str())->Get<TF1>("A_corr");
  ...
  for (int i = 0; i < 3; i++) {
    ...
    std::cout << "i: " << i << " | i30: " << i30 << " | i40: " << i40
              << " | i45: " << i45 << std::endl;
    std::cout << "Opening files for " << drift_arr[i]
              << " mm drift:" << std::endl;
    ...
    // Rename histograms
    th2f50->SetTitle(
        Form("#varphi = %d#circ | %d mm drift;Track length in cluster "
    ...
    std::cout << "TF1 parameters for drift " << drift_arr[i]
              << " mm:" << std::endl;
    ...
    ptf1CorrFunc30->SetTitle(
        ";Track length in cluster [mm];Amplitude [ADC counts/cm]");
    ptf1CorrFunc40->SetTitle(
        ";Track length in cluster [mm];Amplitude [ADC counts/cm]");
    ptf1CorrFunc45->SetTitle(
        ";Track length in cluster [mm];Amplitude [ADC counts/cm]");
    ...
    for (TF1 *ptf1 : vptf1CorrFuncTmp) {
      ...
      for (int j = 0; j < npoint; j++) {
        ptgCorrFunc->SetPoint(
            j, j / (float)npoint * Lmax,
  ...
  for (int i = 0; i < 5; i++) {
    ...
    tg->SetMinimum(0);
    tg->SetTitle(
        ";Track length in cluster [mm];F_{ref}(10.19)/F_{ref}(L_{cluster})");
    ptf1CorrFuncRef->SetParameter(0,
                                  ptf1CorrFuncRef->GetParameter(0) + i * 100);
...
void DrawOut_corrections() {
  ...
  v_tf1[0]->SetTitle(
      "Correction functions 30#circ;L_{cluster} (mm);WF (ADC count)");
  ...
  v_tf1[0]->SetTitle(
      "Correction functions 40#circ;L_{cluster} (mm);WF (ADC count)");
  leg->AddEntry(v_tf1[0], "HATRecon", "l");
  for (int i = 4; i < 7; i++) {
    v_tf1[0]->Draw();
  ...
  v_tf1[0]->SetTitle(
      "Correction functions 45#circ;L_{cluster} (mm);WF (ADC count)");
  leg->AddEntry(v_tf1[0], "HATRecon", "l");
  for (int i = 7; i < 10; i++) {
    v_tf1[0]->Draw();
  ...
  // legAll->						AddEntry(v_tf1[0],
  // "HATRecon" , "l");
  for (int i = 1; i < 10; i++) {
    v_tf1[1]->Draw();
    // if(i>1)v_tf1[i]->			Draw("same");
    legAll->AddEntry(v_tf1[i],
                     Form("%i#circ %i cm", angle[(i - 1) / 3], z[(i - 1) % 3]),
                     "l");
  }
  for (int i = 1; i < 10; i++)
    v_tf1[i]->Draw("same");
  ...
  for (int i = 1; i < 10; i++) {
    ...
    tg->SetMinimum(0);
    tg->SetTitle(
        "F(L_{cluster})*F_{ref}(11.28)/F_{ref}(L_{cluster});L_{cluster} "
  ...
  for (int i = 0; i < 5; i++) {
    ...
    tg->SetMinimum(0);
    tg->SetTitle(
        "F_{ref}(11.28)/F_{ref}(L_{cluster}) with F_{ref} shifted;L_{cluster} "
        "(mm);ratio");
