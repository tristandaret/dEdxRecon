...
Model_Charge0D::~Model_Charge0D() {
  int VerboseDelete = 0;
  if (VerboseDelete == 1)
    std::cout << " Model_Charge0D Destructor " << std::endl;
}

double Model_Charge0D::Get_Qprim() const { return m_Qprim; }
void Model_Charge0D::Set_Qprim(const double &Qprim) {
  m_Qprim = Qprim;
  SetSecondaries();
}

void Model_Charge0D::Set_Ion(const int &DoIons, const double &Tion,
                             const double &RTrue, const double &RFactor) {
  m_DoIons = DoIons;
  m_Tion = Tion;
  m_RTrue = RTrue;
  m_RFactor = RFactor;
...
void Model_Charge0D::WriteOut() const {
  ...
  std::cout << " m_RC; " << std::setw(13) << std::setprecision(6) << m_RC
            << std::endl;
...
double Model_Charge0D::Get_Qpad(const double &Time, const double &Time0,
  ...
  double ARG = (YH - YTrue) * Inv_SQRT2_Sigma;
  double RESYH = std::erf(ARG);
  ARG = (YL - YTrue) * Inv_SQRT2_Sigma;
  double RESYL = std::erf(ARG);

  ARG = (XH - XTrue) * Inv_SQRT2_Sigma;
  double RESXH = std::erf(ARG);
  ARG = (XL - XTrue) * Inv_SQRT2_Sigma;
  double RESXL = std::erf(ARG);
