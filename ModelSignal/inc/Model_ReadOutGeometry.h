...
public:
  ...
  void
  Set_Xpad_min(const double &Xpad_min); // Set Lower left corner X position (m)
  void
  Set_Ypad_min(const double &Ypad_min); // Set Lower left corner Y position (m)

  void Set_Nx(const int &Nx); // Set Nber of pads along X
  void Set_Ny(const int &Ny); // Set Nber of pads along Y

  // Pad Parameters
  double Get_XcPad(const int &iX, const int &iY,
                   const int &ModuleNber)
      const; // Get Pad(iX,iY) X position (m) of the center
  double Get_YcPad(const int &iX, const int &iY,
                   const int &ModuleNber)
      const; // Get Pad(iX,iY) X position (m) of the center

  double Get_XLPad(const int &iX, const int &iY,
                   const int &ModuleNber)
      const; // Get Pad(iX,iY) X position (m) of the Left	edge
  double Get_YLPad(const int &iX, const int &iY,
                   const int &ModuleNber)
      const; // Get Pad(iX,iY) X position (m) of the Bottom edge

  double Get_XHPad(const int &iX, const int &iY,
                   const int &ModuleNber)
      const; // Get Pad(iX,iY) X position (m) of the Right edge
  double Get_YHPad(const int &iX, const int &iY,
                   const int &ModuleNber)
      const; // Get Pad(iX,iY) X position (m) of the Top edge

  void GetPadEdges(const int &iX, const int &iY, const int &ModuleNber,
